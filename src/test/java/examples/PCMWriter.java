...
public final class PCMWriter {
  private static final byte[] PCM_HEADER = {
    ...
    0x2B,
    0x00,
    0x00, // Samples per seconds
    0x11,
    0x2B,
    0x00,
    0x00, // Avg Bytes per seconds
    4,
    0, // BLock align
    16,
    0, // Bits per sample
  ...
  private void setSoundFormat(int sampleRate, int bitsPerSample, int channels) {
    synchronized (mHeader) {
      ...
      valueToBytes(
          sampleRate * channels * (bitsPerSample >> 3), 4, mHeader, 28); // Avg Bytes per seconds
