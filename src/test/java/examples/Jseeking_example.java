          ...
          }
          {
            ...
            try {
              ...
              for (j = -4096; j < bread + 4096; j++) {
                if (j + (int) ((pos << 1) >> hs) >= 0
                    && (j + (int) ((pos << 1) >> hs)) < ((int) (pcmlength >> hs) << 1)) {
    ...
    try {
      ...
      if (ov.ov_seekable()) {
        ...
        while (i < (int) ((pcmlength >> hs) << 1)) {
          final Jvorbis_pcm vpcm =
              ov.ov_read(bigassbuffer, i, ((int) (pcmlength >> hs) << 1) - i, true, 1, true);
          ...
          System.err.printf(
              "\rloading.... [%d left]              ", (((pcmlength >> hs) << 1) - i));
        }
        ...
        {
          System.err.printf(
              "testing pcm page seeking to random places in %d samples....\n", pcmlength);
        ...
        {
          System.err.printf(
              "testing pcm exact seeking to random places in %f seconds....\n", timelength);
          ...
          for (i = 0; i < 1000; i++) {
            ...
            if (ov.ov_pcm_tell() != ((val >> hs) << hs)) {
              System.err.printf(
                  "Declared position didn't perfectly match request: %d != %d\n",
                  val, ov.ov_pcm_tell());
              ov.ov_clear(); // added to free resources
              System.exit(1); // FIXME exit without free resources
            }
