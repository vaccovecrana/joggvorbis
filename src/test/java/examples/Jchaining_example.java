...
public final class Jchaining_example {
  ...
  public static void main(final String[] args) {
    ...
    try {
      ...
      if (ov.ov_seekable()) {
        System.out.printf(
            "Input bitstream contained %d logical bitstream section(s).\n", ov.ov_streams());
        System.out.printf("Total bitstream samples: %d\n\n", ov.ov_pcm_total(-1));
        System.out.printf(
            "Total bitstream playing time: %d seconds\n\n", (long) ov.ov_time_total(-1));
      ...
      for (i = 0; i < ov.ov_streams(); i++) {
        ...
        System.out.printf(
            "\t\t%dHz %d channels bitrate %dkbps serial number=%d\n",
            vi.rate, vi.channels, ov.ov_bitrate(i) / 1000, ov.ov_serialnumber(i));
