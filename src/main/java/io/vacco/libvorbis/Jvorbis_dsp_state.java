...
import io.vacco.libogg.Joggpack_buffer;
import java.util.Arrays;

/**
 * vorbis_dsp_state buffers the current vorbis audio analysis/synthesis state. The DSP state belongs
 * to a specific logical bitstream
 */
public final class Jvorbis_dsp_state {
  ...
  */
  Jprivate_state backend_state = null;
  //
  final void clear() {
    analysisp = 0;
    vi = null;
  ...
  */
  ...
  public final int vorbis_block_init(final Jvorbis_block vb) {
    vb.clear();
    vb.vd = this;
    // already 0? vb.localalloc = 0;
    // already null? vb.localstore = null;
    if (this.analysisp != 0) {
      final Jvorbis_block_internal vbi = vb.m_internal = new Jvorbis_block_internal();
      vbi.ampmax = -9999;

      final Joggpack_buffer[] packetblob = vbi.packetblob; // java
      for (int i = 0; i < Jvorbis_info.PACKETBLOBS; i++) {
        if (i == Jvorbis_info.PACKETBLOBS / 2) {
          packetblob[i] = vb.opb;
        } else {
          packetblob[i] = new Joggpack_buffer();
        }
        packetblob[i].oggpack_writeinit();
      }
    }
  ...
  /**
   * Analysis side code, but directly related to blocking. Thus it's here and not in analysis.c
   * (which is for analysis transforms only). The init is here because some of it is shared
   */
  private final boolean _vds_shared_init(final Jvorbis_info vinfo, final boolean encp) {
    ...
    clear();
    final Jprivate_state b = this.backend_state = new Jprivate_state();

    this.vi = vinfo;
    b.modebits = Jcodec.ov_ilog(ci.modes - 1);

    b.transform[0] = new Jmdct_lookup[Jvorbis_info.VI_TRANSFORMB];
    b.transform[1] = new Jmdct_lookup[Jvorbis_info.VI_TRANSFORMB];

    /* MDCT is tranform 0 */
    final int[] blocksizes = ci.blocksizes; // java

    b.transform[0][0] = new Jmdct_lookup();
    b.transform[1][0] = new Jmdct_lookup();
    b.transform[0][0].mdct_init(blocksizes[0] >>> hs);
    b.transform[1][0].mdct_init(blocksizes[1] >>> hs);
     ...
     */
    b.window[0] = Jcodec.ov_ilog(blocksizes[0]) - 7;
    b.window[1] = Jcodec.ov_ilog(blocksizes[1]) - 7;
    ...
    if (encp) {
      ...
      Jsmallft.drft_init(b.fft_look[0], blocksizes[0]);
      Jsmallft.drft_init(b.fft_look[1], blocksizes[1]);

      /* finish the codebooks */
      if (ci.fullbooks == null) {
        final Jcodebook[] fullbooks = new Jcodebook[books];
        ci.fullbooks = fullbooks;
        for (int i = 0; i < books; i++) {
          fullbooks[i] = new Jcodebook();
          fullbooks[i].vorbis_book_init_encode(book_param[i]);
        }
      }

      final Jvorbis_look_psy[] psy = new Jvorbis_look_psy[ci.psys];
      b.psy = psy;
      final Jvorbis_info_psy[] psy_param = ci.psy_param; // java
      for (int i = 0; i < ci.psys; i++) {
        psy[i] = new Jvorbis_look_psy();
        final Jvorbis_info_psy p = psy_param[i];
        psy[i]._vp_psy_init(p, ci.psy_g_param, blocksizes[p.blockflag] >> 1, vinfo.rate);
      }
      ...
      /* finish the codebooks */
      if (ci.fullbooks == null) {
        ci.fullbooks = new Jcodebook[books];
        for (int i = 0; i < books; i++) {
          ci.fullbooks[i] = new Jcodebook();
          if (book_param[i] == null
              || ci.fullbooks[i].vorbis_book_init_decode(book_param[i]) != 0) { // abort_books:
            for (i = 0; i < books; i++) {
              if (book_param[i] != null) {
                book_param[i].vorbis_staticbook_destroy();
                book_param[i] = null;
              }
            }
            vorbis_dsp_clear();
    ...
    final Jvorbis_look_floor[] flr = new Jvorbis_look_floor[ci.floors];
    b.flr = flr;
    final Jvorbis_info_floor[] floor_param = ci.floor_param; // java
    final Jvorbis_func_floor[] floor_p = Jcodec._floor_P;
    int[] tmp = ci.floor_type; // java
    for (int i = 0, ie = ci.floors; i < ie; i++) {
      flr[i] = floor_p[tmp[i]].look(this, floor_param[i]);
    }

    final Jvorbis_look_residue[] residue = new Jvorbis_look_residue[ci.residues];
    b.residue = residue;
  ...
  public final boolean vorbis_analysis_init(final Jvorbis_info vinfo) {

    if (_vds_shared_init(vinfo, true)) {
      return true;
    }
    final Jprivate_state b = backend_state;
    b.psy_g_look = vinfo._vp_global_look();

    /* Initialize the envelope state storage */
    b.ve = new Jenvelope_lookup();
    b.ve._ve_envelope_init(vinfo);

    vinfo.vorbis_bitrate_init(b.bms);
  ...
  public final void vorbis_dsp_clear() {
    // if( v != null ) {
    final Jvorbis_info vinfo = this.vi;
    final Jcodec_setup_info ci = (vinfo != null ? vinfo.codec_setup : null);
    Jprivate_state b = this.backend_state;

    if (b != null) {

      if (b.ve != null) {
        b.ve._ve_envelope_clear();
        b.ve = null;
      }

      if (b.transform[0] != null) {
        b.transform[0][0].mdct_clear();
        b.transform[0][0] = null;
        b.transform[0] = null;
      }
      if (b.transform[1] != null) {
        b.transform[1][0].mdct_clear();
      ...
      if (b.flr != null) {
        if (ci != null) {
          for (int i = 0; i < ci.floors; i++) {
            // Jregistry._floor_P[ci.floor_type[i]].free_look( b.flr[i] );
            b.flr[i] = null;
          }
        }
        b.flr = null;
      ...
      if (b.residue != null) {
        if (ci != null) {
          for (int i = 0; i < ci.residues; i++) {
            Jcodec._residue_P[ci.residue_type[i]].free_look(b.residue[i]);
          }
        }
        b.residue = null;
      }
      ...
      b.bms.vorbis_bitrate_clear();

      b.fft_look[0].drft_clear();
      b.fft_look[1].drft_clear();
    }
  ...
  public final Jvorbis_pcm vorbis_analysis_buffer(final int vals) {

    final Jprivate_state b = this.backend_state;
  ...
  // lpc.c
  /**
   * Autocorrelation LPC coeff generation algorithm invented by N. Levinson in 1947, modified by J.
   * Durbin in 1959.
   *
   * <p>Input : n elements of time doamin data<br>
   * Output: m lpc coefficients, excitation energy
   */
  private static float vorbis_lpc_from_data(
      final float[] data, final int offset, final float[] lpci, int n, final int m) {
  ...
  private static void vorbis_lpc_predict(
      final float[] coeff,
      final float[] prime,
      final int offset,
      final int m,
  ...
  private final void _preextrapolate_helper() {
    ...
    if (delta > (order << 1)) {
      ...
      for (int i = 0, channels = this.vi.channels; i < channels; i++) {
        ...
        /* prime as above */
        vorbis_lpc_from_data(work, 0, lpc, delta, order);

        /* #if 0
        				if( this.m_vi.channels == 2 ) {
        					if( i == 0 )
        						Janalysis._analysis_output("predataL", 0, work, 0, this.m_pcm_current - this.m_centerW, false, false, 0 );
        					else
        						Janalysis._analysis_output("predataR", 0, work, 0, this.m_pcm_current - this.m_centerW, false, false, 0 );
        				} else {
        					Janalysis._analysis_output("predata", 0, work, 0, this.m_pcm_current - this.m_centerW, false, false, 0 );
        				}
        #endif */

        /* run the predictor filter */
        vorbis_lpc_predict(lpc, work, delta - order, order, work, delta, this.centerW);
    ...
    if (vals <= 0) {
      ...
      for (int i = 0, channels = vinfo.channels; i < channels; i++) {
        ...
        if (eof_flag > (order << 1)) {
          ...
          vorbis_lpc_from_data(v_pcm_i, eof_flag - n, lpc, n, order);
  ...
  private final int _ve_envelope_search() {
    final Jcodec_setup_info ci = this.vi.codec_setup;
    final Jvorbis_info_psy_global gi = ci.psy_g_param;
    final Jenvelope_lookup ve = this.backend_state.ve;

    final int searchstep = ve.searchstep; // java
    int first = ve.current / searchstep;
    final int last = this.pcm_current / searchstep - Jenvelope_lookup.VE_WIN;
    if (first < 0) {
      first = 0;
    }

    /* make sure we have enough storage to match the PCM */
    if (last + Jenvelope_lookup.VE_WIN + Jenvelope_lookup.VE_POST > ve.storage) {
      ve.storage = last + Jenvelope_lookup.VE_WIN + Jenvelope_lookup.VE_POST; /* be sure */
      ve.mark = Arrays.copyOf(ve.mark, ve.storage);
    }

    final boolean[] mark = ve.mark; // java
    final float[][] pcm = this.pcm_data.pcm; // java
    for (int j = first, js = searchstep * j, jp = j + Jenvelope_lookup.VE_POST;
        j < last;
        j++, js += searchstep) {
      int ret = 0;

      ve.m_stretch++;
      if (ve.m_stretch > Jenvelope_lookup.VE_MAXSTRETCH * 2) {
        ve.m_stretch = Jenvelope_lookup.VE_MAXSTRETCH * 2;
      }

      for (int i = 0, ib = 0, ie = ve.ch; i < ie; i++, ib += Jenvelope_lookup.VE_BANDS) {
        ret |= ve._ve_amp(gi, pcm[i], js, ve.band, ve.filter, ib);
      }
...

    {
      ...
      final int[] blocksizes = ci.blocksizes; // java
      final int testW =
          center_W + ((((blocksizes[this.W] + blocksizes[0]) >>> 1) + blocksizes[1]) >>> 1);

      int j = ve.cursor;

      while (j < ve.current - (searchstep)) {
          ...
          if (j > center_W) {
            ...
            /* if( false ) {
            						if( j > ve.m_curmark ) {
            							...
            							Janalysis._analysis_output_always("pcmL", seq, this.m_pcm[0], 0, this.m_pcm_current, false, false, totalshift);
            							Janalysis._analysis_output_always("pcmR", seq, this.m_pcm[1], 0, this.m_pcm_current, false, false, totalshift);

            							Janalysis._analysis_output_always("markL", seq, this.m_pcm[0], 0, j, false, false, totalshift);
            							Janalysis._analysis_output_always("markR", seq, this.m_pcm[1], 0, j, false, false, totalshift);

            							for( m = 0; m < Jenvelope_lookup.VE_BANDS; m++ ) {
            								...
            								Janalysis._analysis_output_always( buf, seq, marker, 0, this.m_pcm_current, false, false, totalshift);
            							}
            							...
            							Janalysis._analysis_output_always("mark", seq, marker, 0, this.pcm_current, false, false, totalshift);
  ...
  // envelope.c
  private final boolean _ve_envelope_mark() {
    final Jenvelope_lookup ve = this.backend_state.ve;
    final Jcodec_setup_info ci = this.vi.codec_setup;
    final int center_W = this.centerW;
  ...
  private final float _vp_ampmax_decay(float amp) {
    final Jcodec_setup_info ci = this.vi.codec_setup;
  ...
  // block.c
  /**
   * do the deltas, envelope shaping, pre-echo and determine the size of the next block on which to
   * continue analysis
   */
  public final boolean vorbis_analysis_blockout(final Jvorbis_block vb) {
    ...
    final Jcodec_setup_info ci = vinfo.codec_setup;
    final Jprivate_state b = this.backend_state;
    final Jvorbis_look_psy_global g = b.psy_g_look;
    final int[] blocksizes = ci.blocksizes; // java
    ...
    {
      final int bp = _ve_envelope_search();
      if (bp == -1) {
    ...
    g.ampmax = _vp_ampmax_decay(g.ampmax);
    vbi.ampmax = g.ampmax;
  ...
  public final int vorbis_synthesis_restart() {
  ...
  public final boolean vorbis_synthesis_init(final Jvorbis_info vinfo) {
    if (_vds_shared_init(vinfo, false)) {
      vorbis_dsp_clear();
      return true;
    }
  ...
  /**
   * Unlike in analysis, the window is only partially applied for each block. The time domain
   * envelope is not yet handled at the point of calling (as it relies on the previous block).
   */
  public final int vorbis_synthesis_blockin(final Jvorbis_block vb) {
    if (vb == null) {
      return (Jcodec.OV_EINVAL);
    }
    ...
    final int[] blocksizes = ci.blocksizes; // java
    if (vb.pcm != null) {
      /* no pcm to process if vorbis_synthesis_trackonly was called on block */
      ...
      if (this.lW != 0 && this.W != 0) {
        w = Jwindow._vorbis_window_get(b.window[1] - hs);
      } else {
        w = Jwindow._vorbis_window_get(b.window[0] - hs);
      }
      ...
      for (int j = 0, channels = this.vi.channels; j < channels; j++) {
        ...
        if (this.lW != 0) {
          if (this.W != 0) {
            /* large/large */
            // final float[] w = Jwindow._vorbis_window_get( window[1] - hs );
            ...
            /* large/small */
            // final float[] w = Jwindow._vorbis_window_get( window[0] - hs );
    ...
    if (this.granulepos == -1) {
      if (vb.granulepos != -1) {
        ...
        if (b.sample_count > this.granulepos) {
          ...
          int extra =
              (int) (b.sample_count - vb.granulepos); // FIXME converting from int64 to int32
      ...
      if (vb.granulepos != -1 && this.granulepos != vb.granulepos) {
        ...
        if (this.granulepos > vb.granulepos) {
          int extra =
              (int) (this.granulepos - vb.granulepos); // FIXME converting from int64 to int32
   ...
   */
  public final Jvorbis_pcm vorbis_synthesis_pcmout(final boolean is_get_pcm) {
    // Jvorbis_info vi = v.vi;
  ...
  public final int vorbis_synthesis_read(final int n) {
  ...
  /**
   * intended for use with a specific vorbisfile feature; we want access to the [usually
   * synthetic/postextrapolated] buffer and lapping at the end of a decode cycle, specifically, a
   * half-short-block worth. This funtion works like pcmout above, except it will also expose this
   * implicit buffer data not normally decoded.
   */
  public final Jvorbis_pcm vorbis_synthesis_lapout(final boolean is_get_pcm) {
  ...
  final float[] vorbis_window(final int w) {
    final int window = this.backend_state.window[w];

    if (window - 1 < 0) {
      return null;
    }
    return Jwindow._vorbis_window_get(window - this.vi.codec_setup.halfrate_flag);
  }

  // info.c
  public final int vorbis_analysis_headerout(
      final Jvorbis_comment vc,
      final Jogg_packet op,
      final Jogg_packet op_comm,
      final Jogg_packet op_code) {
    int ret = Jcodec.OV_EIMPL;
    final Jvorbis_info vinfo = this.vi;
    ...
    if (b == null || vi.channels <= 0 || vi.channels > 256) {
      b = null;
      ret = Jcodec.OV_EFAULT;
    } else {

      /* first header packet **********************************************/

      opb.oggpack_writeinit();
      if (vinfo._vorbis_pack_info(opb) == 0) {

        /* build the packet */
        if (b.header != null) {
          b.header = null;
        }
        b.header = new byte[opb.oggpack_bytes()];
        ...
        opb.oggpack_reset();
        if (vc._vorbis_pack_comment(opb) == 0) {

          if (b.header1 != null) {
            b.header1 = null;
          }
          b.header1 = new byte[opb.oggpack_bytes()];
          ...
          opb.oggpack_reset();
          if (vinfo._vorbis_pack_books(opb) == 0) {

            if (b.header2 != null) {
              b.header2 = null;
            }
            b.header2 = new byte[opb.oggpack_bytes()];
  ...
  // bitrate.c
  public final boolean vorbis_bitrate_flushpacket(final Jogg_packet op) {
    final Jbitrate_manager_state bm = this.backend_state.bms;
    final Jvorbis_block vb = bm.vb;
    int choice = Jvorbis_info.PACKETBLOBS / 2;
    if (vb == null) {
      return false;
    }
    ...
    if (op != null) {
      ...
      final Joggpack_buffer opb = vbi.packetblob[choice]; // java
      op.packet_base = opb.oggpack_get_buffer();
      op.packet = 0;
      op.bytes = opb.oggpack_bytes();
      op.b_o_s = false;
      op.e_o_s = vb.eofflag;
