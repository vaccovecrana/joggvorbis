...
import io.vacco.libvorbis.modes.*;
import java.io.UnsupportedEncodingException;

/**
 * vorbis_info contains all the setup information specific to the specific compression/decompression
 * mode in progress (eg, psychoacoustic settings, channel setup, options, codebook etc). vorbis_info
 * and substructures are in backends.h.
 */
public final class Jvorbis_info {
  ...
  */
  ...
  private static final Jvorbis_info_mode _mode_template[] = { // [2]
    new Jvorbis_info_mode(0, 0, 0, 0), new Jvorbis_info_mode(1, 0, 0, 1)
  };
  ...
  // info.c
  /**
   * blocksize 0 is guaranteed to be short, 1 is guaranteed to be long. They may be equal, but short
   * will never ge greater than long
   */
  ...
  static final void _v_readstring(
      final Joggpack_buffer o, final String[] strings, final int offset, int bytes) {
    final byte[] buf = new byte[bytes];
    for (int i = 0; bytes-- > 0; ) {
      buf[i++] = (byte) o.oggpack_read(8);
    }
    try {
      strings[offset] = new String(buf, Jvorbis_comment.TEXT_ENCODING);
  ...
  final int _vorbis_pack_info(final Joggpack_buffer opb) {
    final Jcodec_setup_info ci = this.codec_setup;
    if (ci == null || ci.blocksizes[0] < 64 || ci.blocksizes[1] < ci.blocksizes[0]) {
      return (Jcodec.OV_EFAULT);
    }
  ...
  /**
   * The Vorbis header is in three packets; the initial small packet in the first page that
   * identifies basic parameters, a second packet with bitstream comments and a third packet that
   * holds the codebook.
   */
  public static final int vorbis_synthesis_headerin(
      final Jvorbis_info vi, final Jvorbis_comment vc, final Jogg_packet op) {
  ...
  // vorbisenc.c
  private final void vorbis_encode_floor_setup(
      final double s, // FIXME why int s?
  ...
  private final void vorbis_encode_global_psych_setup(
      final double s, final Jvorbis_info_psy_global[] in, final double[] x) {
    int is = (int) s;
    double ds = s - is;
    final Jcodec_setup_info ci = this.codec_setup;
    ...
    for (int i = 0; i < 4; i++) {
      ...
      g_postecho_thresh[i] =
          (float) (in_is_postecho_thresh[i] * ds1 + in_is_postecho_thresh1[i] * ds);
    }
    g.ampmax_att_per_sec = (float) ci.hi.amplitude_track_dBpersec;
    return;
  }

  private final void vorbis_encode_global_stereo(
      final Jhighlevel_encode_setup hi, final Jadj_stereo[] p) {
    ...
    if (p != null) {
      ...
      if (hi.managed) {
        ...
        float kHz = (float) (a_kHz[PACKETBLOBS / 2] * ds1 + (double) a1_kHz[PACKETBLOBS / 2] * ds);
        ...
        kHz =
            (float)
                (a_lowpasskHz[PACKETBLOBS / 2] * ds1
  ...
  private final void vorbis_encode_psyset_setup(
      final double s,
      final int[] nn_start,
      final int[] nn_partition,
      final double[] nn_thresh,
      final int block) {
    final Jcodec_setup_info ci = this.codec_setup;
  ...
  private final void vorbis_encode_tonemask_setup(
      final double s,
      final int block,
      final Jatt3[] att,
      final int[] max,
      final Jvp_adjblock[] in) {
    final int is = (int) s;
    final double ds = s - is;
  ...
  private final void vorbis_encode_compand_setup(
      final double s, final int block, final Jcompandblock[] in, final double[] x) {
    int is = (int) s;
    double ds = s - is;
    final Jcodec_setup_info ci = this.codec_setup;
  ...
  private final void vorbis_encode_peak_setup(
      final double s, final int block, final int[] suppress) {
    final int is = (int) s;
    final double ds = s - is;
    final Jvorbis_info_psy p = this.codec_setup.psy_param[block];
  ...
  private final void vorbis_encode_noisebias_setup(
      final double s,
      final int block,
      final int[] suppress,
      final Jnoise3[] in,
      final Jnoiseguard[] guard,
  ...
  private static final int book_dup_or_new(
      final Jcodec_setup_info ci, final Jstatic_codebook book) {
  ...
  private final void vorbis_encode_blocksize_setup(
      final double s, final int[] shortb, final int[] longb) {
  ...
  private final void vorbis_encode_residue_setup(
      final int number, final int block, final Jvorbis_residue_template res) {
    ...
    /* lowpass setup/pointlimit */
    {
      ...
      if (ci.residue_type[number] == 2) {
        ...
        r.end =
            (int) ((freq / nyq * blocksize * ch) / r.grouping + .9)
                * /* round up only if we're well past */ r.grouping;
        ...
        r.end =
            (int) ((freq / nyq * blocksize) / r.grouping + .9)
                * /* round up only if we're well past */ r.grouping;
        /* the blocksize and grouping may disagree at the end */
        if (r.end > blocksize) {
  ...
  private final void vorbis_encode_map_n_res_setup(
      final double s, final Jvorbis_mapping_template[] maps) {
  ...
  private static final void get_setup_template(
      final int ch,
      final int srate,
      double req,
      final boolean q_or_bitrate,
    ...
    for (int i = 0; (st = setup_list[i]) != null; i++) { // java: while changed to for
      if (st.coupling_restriction == -1 || st.coupling_restriction == ch) {
        if (srate >= st.samplerate_min_restriction && srate <= st.samplerate_max_restriction) {
          final int mappings = st.mappings;
          final double[] map = (q_or_bitrate ? st.rate_mapping : st.quality_mapping);
  ...
  /**
   * This function performs the last stage of three-step encoding setup, as described in the API
   * overview under managed bitrate modes.
   *
   * <p>Before this function is called, the {@link Jvorbis_info} struct should be initialized by
   * using vorbis_info_init() from the libvorbis API, one of {@link #vorbis_encode_setup_managed} or
   * {@link #vorbis_encode_setup_vbr} called to initialize the high-level encoding setup, and {@link
   * #vorbis_encode_ctl} called if necessary to make encoding setup changes.
   * vorbis_encode_setup_init() finalizes the highlevel encoding structure into a complete encoding
  ...
  public final int vorbis_encode_setup_init() {
    ...
    for (int i = 0, ie = setup.floor_mappings; i < ie; i++) {
      vorbis_encode_floor_setup(
          hi.base_setting, setup.floor_books, setup.floor_params, floor_mapping_list[i]);
    }
  ...
  private final void vorbis_encode_setup_setting(final int nchannels, final int srate) {
    ...
    if (!hi.lowpass_altered) {
      hi.lowpass_kHz = setup.psy_lowpass[is] * ds1 + setup.psy_lowpass[is + 1] * ds;
    }

    hi.ath_floating_dB = setup.psy_ath_float[is] * ds1 + setup.psy_ath_float[is + 1] * ds;
    hi.ath_absolute_dB = setup.psy_ath_abs[is] * ds1 + setup.psy_ath_abs[is + 1] * ds;
   ...
   * It functions similarly to the one-step setup performed by {@link #vorbis_encode_init_vbr} but
   * allows an application to make further encode setup tweaks using {@link #vorbis_encode_ctl}
   * before finally calling {@link #vorbis_encode_setup_init} to complete the setup process.
   *
   * <p>Before this function is called, the {@link Jvorbis_info} struct should be initialized by
  ...
  /**
   * This is the primary function within libvorbisenc for setting up variable bitrate ("quality"
   * based) modes.
   *
   * <p>Before this function is called, the vorbis_info struct should be initialized by using
  ...
  /**
   * This function performs step-one of a three-step bitrate-managed encode setup. It functions
   * similarly to the one-step setup performed by {@link #vorbis_encode_init} but allows an
   * application to make further encode setup tweaks using {@link #vorbis_encode_ctl} before finally
   * calling {@link #vorbis_encode_setup_init} to complete the setup process.
   *
   * <p>Before this function is called, the {@link Jvorbis_info} struct should be initialized by
   * using vorbis_info_init() from the libvorbis API. After encoding, vorbis_info_clear() should be
   * called.
   *
   * <p>The max_bitrate, nominal_bitrate, and min_bitrate settings are used to set constraints for
  ...
  public final int vorbis_encode_setup_managed(
      final int nchannels,
      final int srate,
      final int max_bitrate,
      int nominal_bitrate,
      final int min_bitrate) {
    if (srate <= 0) {
      return Jcodec.OV_EINVAL;
    }
  ...
  public final int vorbis_encode_init(
      ...
      final int srate,
      final int max_bitrate,
      final int nominal_bitrate,
      final int min_bitrate) {

    int ret =
        vorbis_encode_setup_managed(nchannels, srate, max_bitrate, nominal_bitrate, min_bitrate);
    if (ret != 0) {
      vorbis_info_clear();
      return (ret);
    }
  ...
  /**
   * This function implements a generic interface to miscellaneous encoder settings similar to the
   * classic UNIX 'ioctl()' system call. Applications may use vorbis_encode_ctl() to query or set
   * bitrate management or quality mode details by using one of several request arguments detailed
   * below. vorbis_encode_ctl() must be called after one of vorbis_encode_setup_managed() or
      ...
      case Jencctlcodes.OV_ECTL_RATEMANAGE_HARD:
        {
          ...
          if (ai == null) {
            ...
            hi.bitrate_reservoir =
                (int) (ai.bitrate_hard_window * (hi.bitrate_max + hi.bitrate_min) * .5);
          }
      ...
      case Jencctlcodes.OV_ECTL_COUPLING_SET:
        {
          ...
          /*new_template = */ get_setup_template(
              hi.coupling_p ? this.channels : -1, this.rate, hi.req, hi.managed, hi);
