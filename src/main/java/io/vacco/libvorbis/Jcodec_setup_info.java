...
/**
 * codec_setup_info contains all the setup information specific to the specific
 * compression/decompression mode in progress (eg, psychoacoustic settings, channel setup, options,
 * codebook etc).
 */
final class Jcodec_setup_info {
  ...
  /**
   * modes are the primary means of supporting on-the-fly different blocksizes, different channel
   * mappings (LR or M/A), different residue backends, etc. Each mode consists of a blocksize flag
   * and a mapping (along with the mapping setup
   */
  int modes = 0;
  ...
  /**
   * used only by vorbisenc.c. It's a highly redundant structure, but improves clarity of program
   * flow.
   */
