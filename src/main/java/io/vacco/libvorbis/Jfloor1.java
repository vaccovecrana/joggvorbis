 ...
 * </pre>
 */
final class Jfloor1 extends Jvorbis_func_floor implements Icomparator {
  ...
  private static final class Jlsfit_acc {
    private int x0 = 0;
    private int x1 = 0;
  ...
  /***********************************************/
  // change call floor1_free_info to Jvorbis_info_floor1 = null;
  /*static void floor1_free_info(vorbis_info_floor *i)
  	Jvorbis_info_floor1 info = (vorbis_info_floor1)i;
  	if( info != null ) {
  ...
  final void pack(final Jvorbis_info_floor i, final Joggpack_buffer opb) {
    final Jvorbis_info_floor1 info = (Jvorbis_info_floor1) i;
    ...
    opb.oggpack_write(partitions, 5); /* only 0 to 31 legal */
    final int[] partitionclass = info.partitionclass; // java
    for (int j = 0; j < partitions; j++) {
      final int k = partitionclass[j];
      opb.oggpack_write(k, 4); /* only 0 to 15 legal */
      if (maxclass < k) {
        maxclass = k;
      }
    }
    ...
    for (int j = 0; j <= maxclass; j++) {
      opb.oggpack_write(class_dim[j] - 1, 3); /* 1 to 8 */
      int count = class_subs[j];
      opb.oggpack_write(count, 2); /* 0 to 3 */
      if (count != 0) {
        opb.oggpack_write(class_book[j], 8);
      }
      count = 1 << count;
      final int[] class_subbook_j = class_subbook[j]; // java
      for (int k = 0; k < count; k++) {
        opb.oggpack_write(class_subbook_j[k] + 1, 8);
      }
    }

    /* save out the post list */
    opb.oggpack_write(info.mult - 1, 2); /* only 1,2,3,4 legal now */
    /* maxposit cannot legally be less than 1; this is encode-side, we
    can assume our setup is OK */
    opb.oggpack_write(Jcodec.ov_ilog(maxposit - 1), 4);
    final int rangebits = Jcodec.ov_ilog(maxposit - 1);

    int count = 2;
    for (int j = 0, k = 2; j < partitions; j++) {
      count += class_dim[partitionclass[j]];
      for (; k < count; k++) {
        opb.oggpack_write(postlist[k], rangebits);
      }
    }
  }
  ...
  private Icomparator sort_comparator_set(final int[] dim) {
    mData = dim;
    return this;
  }
  ...
  // private final Icomparator icomp = this;

  @Override
  // vorbis_info_floor *floor1_unpack(vorbis_info *vi,oggpack_buffer *opb){
  final Jvorbis_info_floor unpack(final Jvorbis_info vi, final Joggpack_buffer opb) {

    int maxclass = -1;

    final Jvorbis_info_floor1 info = new Jvorbis_info_floor1();
    /* read partitions */
    final int partitions = opb.oggpack_read(5); /* only 0 to 31 legal */
    info.partitions = partitions;
    final int[] partitionclass = info.partitionclass; // java
    for (int j = 0; j < partitions; j++) {
      final int k = opb.oggpack_read(4); /* only 0 to 15 legal */
      partitionclass[j] = k;
      if (k < 0) { // goto err_out;
    ...
    final int[][] class_subbook = info.class_subbook; // java
    for (int j = 0; j <= maxclass; j++) {
      class_dim[j] = opb.oggpack_read(3) + 1; /* 1 to 8 */
      int count = opb.oggpack_read(2); /* 0,1,2,3 bits */
      class_subs[j] = count;
      ...
      if (count != 0) {
        class_book[j] = opb.oggpack_read(8);
      }
    ...
    /* read the post list */
    info.mult = opb.oggpack_read(2) + 1; /* only 1,2,3,4 legal now */
    final int rangebits = opb.oggpack_read(4);
    ...
    for (int j = 0, k = 2; j < partitions; j++) {
      count += class_dim[partitionclass[j]];
      if (count > Jvorbis_info_floor.VIF_POSIT + 2) { // goto err_out;
        // info = null;// floor1_free_info( info );
        return (null);
      }
      for (; k < count; k++) {
        final int t = postlist[k] = opb.oggpack_read(rangebits);
  ...
  final Jvorbis_look_floor look(final Jvorbis_dsp_state vd, final Jvorbis_info_floor in) {

    final Jvorbis_info_floor1 info = (Jvorbis_info_floor1) in;
    final Jvorbis_look_floor1 look = new Jvorbis_look_floor1();
    int n = 0;
  ...
  private static int vorbis_dBquant(final float x) {
  ...
  private static void render_line(
      int n, int x0, final int x1, int y0, final int y1, final float[] d) {
    final int dy = y1 - y0;
    final int adx = x1 - x0;
    int ady = (dy < 0) ? -dy : dy;
  ...
  private static void render_line0(
      int n, int x0, final int x1, int y0, final int y1, final int[] d) {
    final int dy = y1 - y0;
    final int adx = x1 - x0;
    int ady = (dy < 0) ? -dy : dy;
    final int base = dy / adx;
  ...
  private static int accumulate_fit(
      final float[] flr,
      final float[] mdct,
      int offset,
      final int x0,
      ...
      final Jlsfit_acc a,
      final int n,
      final Jvorbis_info_floor1 info) {
    int i;
    ...
    for (i = x0, offset += x0; i <= x1; i++) {
      final int quantized = vorbis_dBquant(flr[i]);
      if (quantized != 0) {
  ...
  private static boolean fit_line(
      final Jlsfit_acc[] a,
      final int offset,
      int fits,
      final int[] y0_y1,
  ...
  private static boolean inspect_error(
      int x0,
      final int x1,
      int y0,
      int y1,
      final float[] mask,
      final float[] logfft,
      final int mdct, // XXX logfft can be removed and changed to mask
      final Jvorbis_info_floor1 info) {
    final int dy = y1 - y0;
    final int adx = x1 - x0;
    int ady = (dy < 0) ? -dy : dy;
    ...
    final int sy = (dy < 0 ? base - 1 : base + 1);
    int err = 0;
    int val = vorbis_dBquant(mask[x0]);
    int mse = 0;
    int n = 0;
  ...
  static int[] floor1_fit(
      final Jvorbis_block vb,
      final Jvorbis_look_floor1 look,
      final float[] logfft, // XXX logfft can be removed and changed to logmask
      final int logmdct, /* in */
      final float[] logmask) {
    int i, j;
    final Jvorbis_info_floor1 info = look.vi;
    final int n = look.n;
    final int posts = look.posts;
    int nonzero = 0;
    final Jlsfit_acc[] fits = new Jlsfit_acc[Jvorbis_info_floor.VIF_POSIT + 1];
    for (i = 0; i < Jvorbis_info_floor.VIF_POSIT + 1; i++) {
      fits[i] = new Jlsfit_acc();
    }
    final int[] fit_valueA =
        new int[Jvorbis_info_floor.VIF_POSIT + 2]; /* index by range list position */
    final int[] fit_valueB =
        new int[Jvorbis_info_floor.VIF_POSIT + 2]; /* index by range list position */

    final int[] loneighbor =
        new int[Jvorbis_info_floor.VIF_POSIT + 2]; /* sorted index of range list position (+2) */
    final int[] hineighbor = new int[Jvorbis_info_floor.VIF_POSIT + 2];
    int[] output = null;
    final int[] memo = new int[Jvorbis_info_floor.VIF_POSIT + 2];
  ...
  static int[] floor1_interpolate_fit(
      final Jvorbis_block vb,
      final Jvorbis_look_floor1 look,
      final int[] A,
      final int[] B,
  ...
  static boolean floor1_encode(
      final Joggpack_buffer opb,
      final Jvorbis_block vb,
      final Jvorbis_look_floor1 look,
      final int[] post,
      final int[] ilogmask) {
    ...
    if (post != null) {
      ...
      final int[] out = new int[Jvorbis_info_floor.VIF_POSIT + 2];
      out[0] = post[0];
      out[1] = post[1];
      ...
      for (int i = 2; i < posts; i++) {
        ...
        if ((post[i] & 0x8000) != 0 || (predicted == post[i])) {
          ...
          final int headroom =
              (look.quant_q - predicted < predicted ? look.quant_q - predicted : predicted);
      ...
      /* beginning/end post */
      look.frames++;
      final int ii = Jcodec.ov_ilog(look.quant_q - 1);
      look.postbits += ii << 1;
      opb.oggpack_write(out[0], ii);
      opb.oggpack_write(out[1], ii);

      final Jcodec_setup_info ci = vb.vd.vi.codec_setup;
      final Jstatic_codebook[] sbooks = ci.book_param;
      final Jcodebook[] books = ci.fullbooks;
      ...
      for (int i = 0, j = 2, ie = info.partitions; i < ie; i++) {
        ...
        if (csubbits != 0) {
          ...
          /* write it */
          look.phrasebits += books[info.class_book[iclass]].vorbis_book_encode(cval, opb);
        ...
        for (int k = 0; k < cdim; k++) {
          ...
          if (book >= 0) {
            ...
            if (out[j + k] < books[book].entries) {
              look.postbits += books[book].vorbis_book_encode(out[j + k], opb);
              /*else
              fprintf( stderr, "+!" );*/
            }
  ...
  final Object inverse1(final Jvorbis_block vb, final Jvorbis_look_floor in) {
    /* unpack wrapped/predicted values from stream */
    final Joggpack_buffer opb = vb.opb; // java
    if (opb.oggpack_read(1) == 1) {
      final Jvorbis_look_floor1 look = (Jvorbis_look_floor1) in;
      final int[] fit_value = new int[look.posts];

      final int ii = Jcodec.ov_ilog(look.quant_q - 1);
      fit_value[0] = opb.oggpack_read(ii);
      fit_value[1] = opb.oggpack_read(ii);

      final Jvorbis_info_floor1 info = look.vi;
      final Jcodebook[] books = vb.vd.vi.codec_setup.fullbooks;
      /* partition by partition */
      final int[] partitionclass = info.partitionclass; // java
      final int[] class_dim = info.class_dim; // java
      final int[] class_subs = info.class_subs; // java
      ...
      for (int i = 0, j = 2, ie = info.partitions; i < ie; i++) {
        ...
        if (csubbits != 0) {
          cval = books[info.class_book[iclass]].vorbis_book_decode(opb);
        ...
        for (int k = 0; k < cdim; k++) {
          ...
          cval >>= csubbits;
          if (book >= 0) {
            if ((fit_value[j + k] = books[book].vorbis_book_decode(opb)) == -1) {
              return null; // goto eop;
            }
          } else {
            fit_value[j + k] = 0;
  ...
  @Override
  final boolean inverse2(
      final Jvorbis_block vb, final Jvorbis_look_floor in, final Object memo, final float[] out) {
