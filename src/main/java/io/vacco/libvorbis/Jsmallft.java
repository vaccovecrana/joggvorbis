...
/**
 * FFT implementation from OggSquish, minus cosine transforms, minus all but radix 2/4 case. In
 * Vorbis we only need this cut-down version.
 *
 * <p>To do more than just power-of-two sized vectors, see the full version I wrote for NetLib.
 *
 * <p>Note that the packing is a little strange; rather than the FFT r/i packing following R_0, I_n,
 * R_1, I_1, R_2, I_2 ... R_n-1, I_n-1, it follows R_0, R_1, I_1, R_2, I_2 ... R_n-1, I_n-1, I_n
 * like the FORTRAN version
 */
final class Jsmallft {
      ...
      if (ip != 4) { // goto L102;
        ...
        if (na != 0) {
          dradf4(
              ...
              wa_offset + iw,
              wa_offset + ix2,
              wa_offset + ix3); // fix for wa+iw-1,wa+ix2-1,wa+ix3-1
        } else {
          dradf4(
              ido,
              l1,
  ...
  */
  static void drft_forward(final Jdrft_lookup l, final float[] data) {
    if (l.n == 1) {
      return;
    }
  ...
  static void drft_backward(Jdrft_lookup l, float[] data) {
  	if( l.n == 1 ) return;
  ...
  static void drft_init(final Jdrft_lookup l, final int n) {
    l.n = n;
