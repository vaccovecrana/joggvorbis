...
final class Jcodebook implements Icomparator {
  ...
  /**
   * list of bitstream codewords for each entry c-version: ogg_uint32_t *codelist; java to compare
   * must use: long val = (((long)codelist[offset]) & 0xffffffffL);
   */
  int[] codelist;
  ...
  // sharedbook.c
  /**
   * given a list of word lengths, generate a list of codewords. Works for length ordered or
   * unordered, always assigns the lowest valued codewords first. Extended to handle unused entries
   * (length 0)
   */
  ...
  final int vorbis_book_encode(final int a, final Joggpack_buffer b) {
    if (a < 0 || a >= this.c.entries) {
      return (0);
    }
    b.oggpack_write(this.codelist[a], this.c.lengthlist[a]);
    return (this.c.lengthlist[a]);
  }

  /**
   * the 'eliminate the decode tree' optimization actually requires the codewords to be MSb first,
   * not LSb. This is an annoying inelegancy (and one of the first places where carefully thought
   * out design turned out to be wrong; Vorbis II and future Ogg codecs should go to an MSb
   * bitpacker), but not actually the huge hit it appears to be. The first-stage decode table
   * catches most words so that bitreverse is not in the main execution path.
   */
  ...
  final int decode_packed_entry_number(final Joggpack_buffer b) {
    int read = this.dec_maxlength;
    int lo, hi;
    int lok = b.oggpack_look(this.dec_firsttablen);

    if (lok >= 0) {
      final int entry =
          this.dec_firsttable[lok]; // FIXME is this correct? hidden convert uint32 to long
      if ((entry & 0x80000000) != 0) {
        lo = (entry >>> 15) & 0x7fff;
        hi = this.used_entries - (entry & 0x7fff);
      } else {
        b.oggpack_adv(this.dec_codelengths[entry - 1]);
        return (entry - 1);
      }
    } else {
      lo = 0;
    ...
    /* bisect search for the codeword in the ordered list */
    {
      ...
      if (this.dec_codelengths[lo] <= read) {
        b.oggpack_adv(this.dec_codelengths[lo]);
        return (lo);
      }
    }
  ...
  /**
   * Decode side is specced and easier, because we don't need to find matches using different
   * criteria; we simply read and map. There are two things we need to do 'depending':
   *
   * <p>We may need to support interleave. We don't really, but it's convenient to do it here rather
   * than rebuild the vector later.
   *
   * <p>Cascades may be additive or multiplicitive; this is not inherent in the codebook, but set in
   * the code using the codebook. Like interleaving, it's easiest to do it here.<br>
   ...
   */
  final int vorbis_book_decode(final Joggpack_buffer b) {
  ...
  final void vorbis_book_clear() {
    ...
    */
    clear(); // memset(b,0,sizeof(*b));
  }

  final int vorbis_book_init_encode(final Jstatic_codebook s) {
    ...
    this.minval = (int) Math.rint(Jcodec._float32_unpack(s.q_min));
    this.delta = (int) Math.rint(Jcodec._float32_unpack(s.q_delta));
  ...
  private final Icomparator sort_comparator_set(final int[] a) {
    this.d = a;
    return this;
  }
  ...
  // private final Icomparator sort32a = this;

  /** decode codebook arrangement is more heavily optimized than encode */
  final int vorbis_book_init_decode(final Jstatic_codebook s) {
    clear();
    ...
    if (n > 0) {
      ...
      if (n == 1 && this.dec_maxlength == 1) {
        ...
        this.dec_firsttablen = Jcodec.ov_ilog(this.used_entries) - 4; /* this is magic */
  ...
  // FIXME never used private final int vorbis_book_codeword(int entry) {
  //  if( this.c != null ) /* only use with encode; decode optimizations are allowed to break this
  // */
  //    return this.m_codelist[entry];// FIXME is this a bug? using hidden convert from uint32 to
  // long
  //  return -1;
  // }

  // FIXME never used private final int vorbis_book_codelen(int entry) {
  //  if( this.c != null ) /* only use with encode; decode optimizations are allowed to break this
  // */
  ...
  /**
   * unlike the others, we guard against n not being an integer number of <dim> internally rather
   * than in the upper layer (called only by floor0)
   */
  final int vorbis_book_decodev_set(final float[] a, final Joggpack_buffer b, final int n) {
    if (this.used_entries > 0) {
  ...
  final int vorbis_book_decodevv_add(
      final float[][] a, final int offset, final int ch, final Joggpack_buffer b, int n) {
    int chptr = 0;
    if (this.used_entries > 0) {
      n += offset;
      n /= ch;
