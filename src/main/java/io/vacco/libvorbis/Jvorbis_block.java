 ...
 * it belongs to a specific logical bitstream, but is independent from other vorbis_blocks belonging
 * to that logical bitstream.
 */
public final class Jvorbis_block {
  ...
  */
  ...
  /**
   * used to track pcm position without actually performing decode. Useful for sequential 'fast
   * forward'
   */
  ...
  public final int vorbis_bitrate_addblock() {
    ...
    if (bm.avg_bitsper > 0) {
      double slew = 0.;
      final int avg_target_bits =
          (this.W != 0 ? bm.avg_bitsper * bm.short_per_long : bm.avg_bitsper);
      ...
      if (avg_reservoir + (this_bits - avg_target_bits) > desired_fill) {
        ...
        while (choice + 1 < Jvorbis_info.PACKETBLOBS
            && this_bits < avg_target_bits
            && avg_reservoir + (this_bits - avg_target_bits) < desired_fill) {
          choice++;
          this_bits = packetblob[choice].oggpack_bytes() << 3; // * 8;
    ...
    if (choice < 0) {
      ...
      final int maxsize =
          (max_target_bits + (bi.reservoir_bits - bm.minmax_reservoir))
              / 8; // java: can be negative, don't use >>
      bm.choice = choice = 0;
      ...
      int minsize =
          (min_target_bits - bm.minmax_reservoir + 7) / 8; // java: can be negative, don't use >>
      if (choice >= Jvorbis_info.PACKETBLOBS) {
