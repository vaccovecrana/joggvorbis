...
/**
 * A 'chained bitstream' is a Vorbis bitstream that contains more than one logical bitstream
 * arranged end to end (the only form of Ogg multiplexing allowed in a Vorbis bitstream; grouping
 * [parallel multiplexing] is not allowed in Vorbis)
 *
 * <p>A Vorbis file can be played beginning to end (streamed) without worrying ahead of time about
 * chaining (see decoder_example.c). If we have the whole file, however, and want random access
 * (seeking/scrubbing) or desire to know the total length/time of a file, we need to account for the
 * possibility of chaining.
 *
 * <p>We can handle things a number of ways; we can determine the entire bitstream structure right
 * off the bat, or find pieces on demand. This example determines and caches structure for the
 * entire bitstream, but builds a virtual decoder on the fly when moving between links in the chain.
 *
 * <p>There are also different ways to implement seeking. Enough information exists in an Ogg
 * bitstream to seek to sample-granularity positions in the output. Or, one can seek by picking some
 * portion of the stream roughly in the desired area if we only want coarse navigation through the
 * stream.
 */
public final class JOggVorbis_File {
  ...
  //
  /**
   * The function prototypes for the callbacks are basically the same as for the stdio functions
   * fread, fseek, fclose, ftell. The one difference is that the FILE * arguments have been replaced
   * with a void * - this is to be used as a pointer to whatever internal data these functions might
   * need. In the stdio case, it's just a FILE * cast to a void *
   *
   * <p>If you use other functions, check the docs for these functions and return the right values.
   * For seek_func(), you *MUST* return -1 if the stream is unseekable
   */
  protected static final class Jov_callbacks {
    ...
    //
    private Jov_callbacks(
        final boolean isRead, final boolean isSeek, final boolean isClose, final boolean isTell) {
  ...
  /**
   * These structs below (OV_CALLBACKS_DEFAULT etc) are defined here as static data. That means that
   * every file which includes this header will get its own copy of these structs whether it uses
   * them or not unless it #defines OV_EXCLUDE_STATIC_CALLBACKS. These static symbols are essential
   * on platforms such as Windows on which several different versions of stdio support may be linked
   * to by different DLLs, and we need to be certain we know which one we're using (the same one as
  ...
  private static final class RandomAccessInputStream extends InputStream {
    ...
    /**
     * Reads up to <code>len</code> bytes of data from this file into an array of bytes. This method
     ...
     *     file, or if the random access file has been closed, or if some other I/O error occurs.
    ...
    /**
     * Reads up to <code>b.length</code> bytes of data from this file into an array of bytes. This
     ...
     * @exception IOException if <code>pos</code> is less than <code>0</code> or if an I/O error
     *     occurs.
     */
  ...
  private final int _seek_helper(final long off) {
    if (this.datasource != null) {
      ...
      if (this.offset != off) {
        if (!this.callbacks.is_seek_supported || _seek_func(this.datasource, off, SEEK_SET) == -1) {
  ...
  /**
   * from the head of the stream, get the next page. boundary specifies if the function is allowed
   * to fetch more data from the stream (and how much) or only use internally buffered data.
   *
  ...
  /**
   * find the latest page beginning before the passed in position. Much dirtier than the above as
   * Ogg doesn't have any backward search linkage. no 'readp' as it will certainly have to read.<br>
  ...
  */
  ...
  private static boolean _lookup_serialno(
      final int s, final int[] serialno_list, final int offset) {
  ...
  private static boolean _lookup_page_serialno(
      final Jogg_page og, final int[] serialno_list, final int offset) {
    final int s = og.ogg_page_serialno();
    return _lookup_serialno(s, serialno_list, offset);
  }

  /**
   * performs the same search as _get_prev_page, but prefers pages of the specified serial number.
   * If a page of the specified serialno is spotted during the seek-back-and-read-forward, it will
   * return the info of last page of the matching serial number instead of the very last page. If no
   * page of the specified serialno is seen, it will return the info of last page and alter
   * *serialno.
   */
  private final long _get_prev_page_serial(
      long begin, final int[] serial_list, final int serial_offset, final Jinout_helper in_out) {
  ...
  private final int _fetch_headers(
      final Jvorbis_info vinfo,
      final Jvorbis_comment vcomment,
    ...
    while (og_ptr.ogg_page_bos()) {
      if (in_out != null) {
        ...
        if (in_out.serialno_list != null) {
          in_out.serialno_list =
              Arrays.copyOf(in_out.serialno_list, in_out.serialno_list.length + 1);
        } else {
      ...
      if (this.ready_state < STREAMSET) {
        ...
        if (oss.ogg_stream_packetout(op) > 0 && Jvorbis_info.vorbis_synthesis_idheader(op)) {
      ...
      /* get next page */
      {
        ...
        if (this.ready_state == STREAMSET && oss.serialno == og_ptr.ogg_page_serialno()) {
          oss.ogg_stream_pagein(og_ptr);
          break;
        }
      }
  ...
  /**
   * Starting from current cursor position, get initial PCM offset of next page. Consumes the page
   * in the process without decoding audio, however this is only called during stream parsing upon
   * seekable open.
   */
  ...
  /**
   * finds each bitstream link one at a time using a bisection search (has to begin by knowing the
   * offset of the lb's initial page). Recurses for each link so it can alloc the link storage after
    ...
    if (_lookup_serialno(endserial, currentno_list, currentno_offset)) {
      ...
      ret =
          _bisect_forward_serialno(
              next, this.offset, finish, endgran, endserial, next_serialno_list, 0, m + 1);
      if (ret != 0) {
        return (ret);
      }
  ...
  private final int _open_seekable2() {
    ...
    if (_bisect_forward_serialno(0, dataoffset, finish, endgran, endserial, this.serialnos, 2, 0)
        < 0) {
      return (Jcodec.OV_EREAD);
    }
   ...
   *     1) got a packet
   */
  private final int _fetch_and_process_packet(Jogg_packet op_in, final int readp, final int spanp) {
    final Jogg_page og = new Jogg_page();
    final Jogg_packet op = new Jogg_packet();
      ...
      if (this.ready_state == INITSET) {
        ...
        while (true) {
          ...
          if (result > 0) {
            ...
            if (this.vb.vorbis_synthesis(op_ptr) == 0) {
              /* lazy check for lazy
              header handling.  The
              ...
              {
                final int oldsamples = this.vd.vorbis_synthesis_pcmout(false).samples;
                ...
                this.samptrack += (this.vd.vorbis_synthesis_pcmout(false).samples << hs);
              ...
              if (granulepos != -1 && !op_ptr.e_o_s) {
                ...
                final int samples = (this.vd.vorbis_synthesis_pcmout(false).samples << hs);
  ...
  private final int _ov_open1(
      final InputStream f, final byte[] initial, final int ibytes, final Jov_callbacks cb) {
    final int offsettest = ((f != null && cb.is_seek_supported) ? _seek_func(f, 0, SEEK_CUR) : -1);
    ...
    if ((ret = _fetch_headers(this.vi[0], this.vc[0], this.inout, null)) < 0) {
      ...
      System.arraycopy(
          this.inout.serialno_list, 0, this.serialnos, 2, this.inout.serialno_list.length);
  ...
  /**
   * inspects the OggVorbis file and finds/documents all the logical bitstreams contained in it.
   * Tries to be tolerant of logical bitstream sections that are truncated/woogie.
   *
   * @return -1) error<br>
   *     0) OK
   */
  public final int ov_open_callbacks(
      final InputStream f, final byte[] initial, final int ibytes, final Jov_callbacks cb) {
  ...
  */
  ...
  /**
   * cheap hack for game usage where downsampling is desirable; there's no need for SRC as we can
   * just do it cheaply in libvorbis.
   */
  ...
  /**
   * Only partially open the vorbis file; test for Vorbisness, and load the headers for the first
   * chain. Do not seek (although test for seekability). Use ov_test_open to finish opening the
   * file, else ov_clear to close/free it. Same return codes as open.
   *
   * <p>Note that vorbisfile does _not_ take ownership of the file if the call fails; the calling
   * applicaiton is responsible for closing the file if this call returns an error.
   */
  public final int ov_test_callbacks(
      final InputStream f, final byte[] initial, final int ibytes, final Jov_callbacks cb) {
  ...
  /**
   * returns the bitrate for a given logical bitstream or the entire physical bitstream. If the file
   * is open for random access, it will find the *actual* average bitrate. If the file is streaming,
   * it returns the nominal bitrate (if set) else the average of the upper/lower bounds (if set)
   * else -1 (unset).
   *
   * <p>If you want the actual bitrate field settings, get them from the vorbis_info structs
   */
  public final int ov_bitrate(int i) {
    ...
    if (i < 0) {
      ...
      if (this.seekable) {
        ...
        return (int)
            Math.rint(((this.offsets[i + 1] - this.dataoffsets[i]) << 3) / ov_time_total(i));
      } else {
  ...
  /**
   * @return the actual bitrate since last call. returns -1 if no additional data to offer since
   *     last call (or at beginning of stream), EINVAL if stream is only partially open
   */
   ...
   *     for i==0 to n OV_EINVAL if the stream is not seekable (we can't know the length) or if
   *     stream is only partially open
   */
   ...
   *     for i==0 to n OV_EINVAL if the stream is not seekable (we can't know the length) or only
   *     partially open
   */
  ...
  /**
   * seek to an offset relative to the *compressed* data. This also scans packets to update the PCM
   * cursor. It will cross a logical bitstream boundary, but only if it can't get any packets out of
  ...
  /**
   * Page granularity seek (faster than sample granularity because we don't do the last bit of
   * decode to find a specific sample).
   *
   * <p>Seek to the last [granule marked] page preceding the specified pos location, such that
   * decoding past the returned point will quickly arrive at the requested position.
   */
      ...
      while (begin < finish) {
        ...
        if (finish - begin < CHUNKSIZE) {
          ...
          bisect =
              begin
                  + (long)
                      ((double) (target - begintime) * (finish - begin) / (endtime - begintime))
                  - CHUNKSIZE;
          ...
          if (result == 0) {
            ...
            while (result > this.dataoffsets[link]) {
              ...
              if (og.ogg_page_serialno() == this.current_serialno
                  && (og.ogg_page_granulepos() > -1 || !og.ogg_page_continued())) {
  ...
  /**
   * seek to a sample offset relative to the decompressed pcm stream returns zero on success,
   * nonzero on failure
   */
  public final int ov_pcm_seek(final long pos) {
    ...
    while (true) {
      ...
      if (ret > 0) {
        ...
        if (this.pcm_offset + ((thisblock + this.vi[0].vorbis_info_blocksize(1)) >> 2) >= pos) {
          break;
        }
  ...
  /**
   * seek to a playback time relative to the decompressed pcm stream returns zero on success,
   * nonzero on failure
   */
  public final int ov_time_seek(final double seconds) {
    ...
    {
      final long target =
          (long) ((double) pcm_total + (seconds - time_total) * (double) this.vi[link].rate);
  ...
  /**
   * tell the current stream offset cursor. Note that seek followed by tell will likely not give the
   * set offset due to caching
   */
  ...
  /**
   * @param link -1) return the vorbis_info struct for the bitstream section currently being decoded
   *     <br>
   *     0-n) to request information for a specific bitstream section
   *     <p>In the case of a non-seekable bitstream, any call returns the current bitstream.<br>
  ...
  /**
   * up to this point, everything could more or less hide the multiple logical bitstream nature of
   * chaining from the toplevel application if the toplevel application didn't particularly care.
   * However, at the point that we actually read audio back, the multiple-section nature must
   * surface: Multiple bitstream sections do not necessarily have to have the same number of
   * channels or sampling rate.
   *
   * <p>ov_read returns the sequential logical bitstream number currently being decoded along with
   * the PCM data in order that the toplevel application can take action on channel/sample rate
   * changes. This number will be incremented even for streamed (non-seekable) streams (for seekable
   * streams, it represents the actual logical bitstream index within the physical bitstream. Note
   * that the accessor functions above are aware of this dichotomy).
   *
   * <p>ov_read_filter is exactly the same as ov_read except that it processes the decoded audio
   * data through a filter before packing it into the requested format. This gives greater accuracy
   * than applying a filter after the audio has been converted into integral PCM.
   *
   * <p>
   *
   ...
   *     <p><code>Jvorbis_pcm.data</code>: set to the logical bitstream number
   */
  @SuppressWarnings("null") // if samples != 0 , than pcm_data not null
  public final Jvorbis_pcm ov_read_filter(
      final byte[] buffer,
      final int off,
      final int length,
      final boolean bigendianp,
      final int word,
      final boolean signed,
      // int[] bitstream moved to Jvorbis_pcm.pcmret
      final Iov_read_filter filter,
      final Object filter_param) {
   ...
   */
  public final Jvorbis_pcm ov_read(
      final byte[] buffer,
      final int off,
      final int length,
      // int[] bitstream moved to Jvorbis_pcm.pcmret
   ...
   *     so the return length is not related to the 'length' passed in, just guaranteed to fit.
   *     <p>Jvorbis_pcm.pcm: pcm_channels<br>
  ...
  private static void _ov_splice(
      final float[][] pcm,
      final float[][] lappcm,
      final int n1,
      final int n2,
  ...
  /**
   * make sure vf is INITSET and that we have a primed buffer; if we're crosslapping at a stream
   * section boundary, this also makes sure we're sanity checking against the right stream
   * information
   */
  ...
  /**
   * grab enough data for lapping from vf; this may be in the form of unreturned, already-decoded
   * pcm, remaining PCM we will need to decode, or synthetic postextrapolation from last packets.
   */
  private final void _ov_getlap(
      final Jvorbis_info vinfo,
      final Jvorbis_dsp_state vdsp,
      final float[][] lappcm,
      final int lapsize) {
  ...
  /**
   * this sets up crosslapping of a sample by using trailing data from sample 1 and lapping it into
   * the windowing buffer of sample 2
   */
