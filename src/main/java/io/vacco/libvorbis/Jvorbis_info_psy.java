...
public final class Jvorbis_info_psy {
  int blockflag = 0;

  float ath_adjatt = 0.0f;
  float ath_maxatt = 0.0f;

  final float[] tone_masteratt = new float[Jvorbis_look_psy.P_NOISECURVES];
  float tone_centerboost = 0.0f;
  float tone_decay = 0.0f;
  float tone_abs_limit = 0.0f;
  float[] toneatt = new float[Jvorbis_look_psy.P_BANDS];
  ...
  float[][] noiseoff = new float[Jvorbis_look_psy.P_NOISECURVES][Jvorbis_look_psy.P_BANDS];
  float[] noisecompand = new float[Jvorbis_look_psy.NOISE_COMPAND_LEVELS];
  ...
  Jvorbis_info_psy(final Jvorbis_info_psy vip) {
  ...
  public Jvorbis_info_psy(
      final int i_blockflag,
      final float f_ath_adjatt,
      final float f_ath_maxatt,
      final float[] f_tone_masteratt,
      final float f_tone_centerboost,
      final float f_tone_decay,
      final float f_tone_abs_limit,
      final float[] f_toneatt,
      final boolean i_noisemaskp,
      final float f_noisemaxsupp,
      final float f_noisewindowlo,
      final float f_noisewindowhi,
      final int i_noisewindowlomin,
      final int i_noisewindowhimin,
      final int i_noisewindowfixed,
      final float[][] f_noiseoff,
      final float[] noisecompand,
      final float f_max_curve_dB,
      final boolean i_normal_p,
      final int i_normal_start,
      final int i_normal_partition,
