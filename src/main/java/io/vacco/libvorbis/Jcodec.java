...
/**
 * libvorbis encodes in two abstraction layers; first we perform DSP and produce a packet (see
 * docs/analysis.txt). The packet is then coded into a framed OggSquish bitstream by the second
 * layer (see docs/framing.txt). Decode is the reverse process; we sync/frame the bitstream and
 * extract individual packets, then decode the packet back into PCM audio.
 *
 * <p>The extra framing/packetizing is used in streaming formats, such as files. Over the net (such
 * as with UDP), the framing and packetization aren't necessary as they're provided by the transport
...
public final class Jcodec {
  ...
  /**
   * seems like major overkill now; the backend numbers will grow into the infrastructure soon
   * enough
   */
  static final Jvorbis_func_floor _floor_P[] = {
    new Jfloor0(), new Jfloor1(),
  };

  static final Jvorbis_func_residue _residue_P[] = {
    new Jresidue0(), new Jresidue1(), new Jresidue2(),
  };
  ...
  */
  ...
  /**
   * The bark scale equations are approximations, since the original table was somewhat hand rolled.
   * The below are chosen to have the best possible fit to the rolled tables, thus their somewhat
   * odd appearance (these are more accurate and over a longer range than the oft-quoted bark
   * equations found in the texts I have). The approximations are valid from 0 - 30kHz (nyquist) or
   * so.
   *
   * <p>all f in Hz, z in Bark
   */
  static float toBARK(final float n) {
    ...
    return (float)
        (13.1 * Math.atan(.00074 * dn) + 2.24 * Math.atan(dn * dn * 1.85e-8) + 1e-4 * dn);
  }
