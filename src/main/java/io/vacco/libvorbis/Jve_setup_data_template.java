...
/**
 * high level configuration information for setting things up step-by-step with the detailed
 * vorbis_encode_ctl interface. There's a fair amount of redundancy such that interactive setup does
 * not directly deal with any vorbis_info or codec_setup_info initialization; it's all stored (until
 * full init) in this highlevel setup, then flushed out to the real codec setup structs later.
 */
public final class Jve_setup_data_template {
  final int mappings;
  ...
  final Jatt3[] psy_tone_masteratt;
  final int[] psy_tone_0dB;
  final int[] psy_tone_dBsuppress;

  final Jvp_adjblock[] psy_tone_adj_impulse;
  final Jvp_adjblock[] psy_tone_adj_long;
  final Jvp_adjblock[] psy_tone_adj_other;

  final Jnoiseguard[] psy_noiseguards;
  final Jnoise3[] psy_noise_bias_impulse;
  final Jnoise3[] psy_noise_bias_padding;
  final Jnoise3[] psy_noise_bias_trans;
  final Jnoise3[] psy_noise_bias_long;
  final int[] psy_noise_dBsuppress;

  final Jcompandblock[] psy_noise_compand;
  ...
  final Jvorbis_info_psy_global[] global_params;
  final double[] global_mapping;
  final Jadj_stereo[] stereo_modes;

  final Jstatic_codebook[][] floor_books;
  final Jvorbis_info_floor1[] floor_params;
  final int floor_mappings;
  final int[][] floor_mapping_list;

  final Jvorbis_mapping_template[] maps;
  //
  public Jve_setup_data_template(
      final int i_mappings,
      ...
      final Jatt3[] pa_psy_tone_masteratt,
      final int[] pi_psy_tone_0dB,
      final int[] pi_psy_tone_dBsuppress,
      final Jvp_adjblock[] pvp_psy_tone_adj_impulse,
      final Jvp_adjblock[] pvp_psy_tone_adj_long,
      final Jvp_adjblock[] pvp_psy_tone_adj_other,
      final Jnoiseguard[] png_psy_noiseguards,
      final Jnoise3[] pn_psy_noise_bias_impulse,
      final Jnoise3[] pn_psy_noise_bias_padding,
      final Jnoise3[] pn_psy_noise_bias_trans,
      final Jnoise3[] pn_psy_noise_bias_long,
      final int[] pi_psy_noise_dBsuppress,
      final Jcompandblock[] pc_psy_noise_compand,
      ...
      final Jvorbis_info_psy_global[] pvip_global_params,
      final double[] pd_global_mapping,
      final Jadj_stereo[] pa_stereo_modes,
      final Jstatic_codebook[][] ps_floor_books,
      final Jvorbis_info_floor1[] pvi_floor_params,
      final int i_floor_mappings,
      final int[][] pi_floor_mapping_list,
      final Jvorbis_mapping_template[] pm_maps) {
    mappings = i_mappings;
