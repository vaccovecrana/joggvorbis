 ...
 * </pre>
 */
class Jresidue1 extends Jresidue0 implements Iencodepart {

  Jresidue1(final boolean isPack, final boolean isClass, final boolean isForward) {
  ...
  private static int local_book_besterror(final Jcodebook book, final int[] a, int offset) {
  ...
  // #endif
  public final int encodepart(
      final Joggpack_buffer opb,
      final int[] vec,
      final int offset,
      final int n,
      final Jcodebook book) { // , final int[] acc) {
    int bits = 0;
    final int dim = book.dim;
    ...
    for (int i = offset; i < step; i += dim) {
      ...
      bits += book.vorbis_book_encode(entry, opb);
    }
  ...
  private static int[][] _01class(
      final Jvorbis_block vb, final Object vl, final int[][] in, final int ch) {
    ...
    for (int i = 0, offset = info.begin; i < partvals; i++, offset += samples_per_partition) {
      ...
      for (int j = 0; j < ch; j++) {
        ...
        for (; k < possible_partitions1; k++) {
          ...
          if (max <= classmetric1[k] && (v < 0 || ent < v)) {
            break;
          }
        }
      ...
      for (int i = 0; i < partvals; ) {
        ...
        if (s == 0) {
          for (int j = 0; j < ch; j++) {
            ...
            /* training hack */
            if (val < entries) {
              look.phrasebits += look.phrasebook.vorbis_book_encode(val, opb);
              // #if 0 /*def TRAIN_RES*/
              //            else
              //              System.err.print("!");
              // #endif
        ...
        for (int k = 0; k < partitions_per_word && i < partvals; k++, i++) {
          ...
          for (int j = 0; j < ch; j++) {
            ...
            if ((secondstages[pw] & (1 << s)) != 0) {
              final Jcodebook statebook = partbooks[pw][s];
              if (statebook != null) {
                final int ret;
                ...
                #else */
                ret = encode.encodepart(opb, in[j], offset, samples_per_partition, statebook);
                // #endif
