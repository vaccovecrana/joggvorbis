...
final class Jvorbis_look_psy implements Icomparator {
  // masking.h
  /**
   * more detailed ATH; the bass if flat to save stressing the floor overly for only a bin or two of
   * savings.
   */
  ...
  private static final float stereo_threshholds[] = {
    0.0f, .5f, 1.0f, 1.5f, 2.5f, 4.5f, 8.5f, 16.5f, 9e10f
  };
  private static final float stereo_threshholds_limited[] = {
    0.0f, .5f, 1.0f, 1.5f, 2.0f, 2.5f, 4.5f, 8.5f, 9e10f
  };
  ...
  /**
   * masking tones from -50 to 0dB, 62.5 through 16kHz at half octaves test tones from -2 octaves to
   * +5 octaves sampled at eighth octaves
   *
   * <p>(Vorbis 0dB, the loudest possible tone, is assumed to be ~100dB SPL for collection of these
   * curves)
   */
  private static final float[][][] tonemasks =
      new float[][][] { // [P_BANDS][6][EHMER_MAX]
        /* 62.5 Hz */
        {
          {
  ...
  //
  private int n = 0;
  private Jvorbis_info_psy vi = null;
  ...
  private static float[][][] setup_tone_curves(
      final float[] curveatt_dB,
      final float binHz,
      final int n,
      final float center_boost,
      final float center_decay_rate) {
    ...
    for (int i = 0; i < P_BANDS; i++) {
      ...
      int bin = (int) Math.floor(Jcodec.fromOC(i * .5f) / binHz);
      int lo_curve = (int) Math.ceil(Jcodec.toOC(bin * binHz + 1) * 2);
      int hi_curve = (int) Math.floor(Jcodec.toOC((bin + 1) * binHz) * 2);
      if (lo_curve > i) {
        lo_curve = i;
      }
      if (lo_curve < 0) {
      ...
      for (int m = 0; m < P_LEVELS; m++) {
        ...
        for (int k = lo_curve; k <= hi_curve; k++) {
          ...
          for (int j = 0; j < EHMER_MAX; j++) {
            ...
            int lo_bin = (int) (Jcodec.fromOC(j8 + k1) / binHz);
            int hi_bin = (int) (Jcodec.fromOC(j8 + k2) / binHz) + 1;
        ...
        if (i + 1 < P_BANDS) {
          ...
          for (int j = 0; j < EHMER_MAX; j++) {
            final float j8 = j * .125f; // java
            int lo_bin = (int) (Jcodec.fromOC(j8 + i1) / binHz);
            int hi_bin = (int) (Jcodec.fromOC(j8 + i2) / binHz) + 1;
        ...
        for (int j = 0; j < EHMER_MAX; j++) {
          bin = (int) (Jcodec.fromOC(j * .125f + i5) / binHz);
          if (bin < 0) {
            r[j + 2] = -999.f;
          } else {
            if (bin >= n) {
  ...
  final void _vp_psy_init(
      final Jvorbis_info_psy vinfo,
    ...
    this.firstoc =
        (int) (Jcodec.toOC(.25f * rate * .5f / size) * (1 << (this.shiftoc + 1)))
            - gi.eighth_octave_lines;
    final int maxoc =
        (int) (Jcodec.toOC((size + .25f) * rate * .5f / size) * (1 << (this.shiftoc + 1)) + .5f);
    ...
    for (int i = 0; i < MAX_ATH - 1; i++) {
      final int endpos = (int) Math.rint(Jcodec.fromOC((i + 1) * .125f - 2.f) * 2 * size / rate);
      float base = ATH[i];
      if (j < endpos) {
    ...
    for (int i = 0; i < size; i++) {
      final float fbark = Jcodec.toBARK((float) (rj * i));
      float b = fbark - vinfo.noisewindowlo; // java

      for (; lo + vinfo.noisewindowlomin < i && Jcodec.toBARK((float) (rj * lo)) < b; lo++) {
        ;
      }

      b = fbark + vinfo.noisewindowhi; // java
      for (;
          hi <= size && (hi < i + vinfo.noisewindowhimin || Jcodec.toBARK((float) (rj * hi)) < b);
          hi++) {
        ;
      }
    ...
    for (int i = 0; i < size; i++) {
      o[i] = (int) (Jcodec.toOC((i + .25f) * .5f * rate / size) * tmp + .5f);
    }

    this.tonecurves =
        setup_tone_curves(
            vinfo.toneatt, rate * .5f / size, size, vinfo.tone_centerboost, vinfo.tone_decay);
    ...
    for (int i = 0; i < size; i++) {
      float halfoc = Jcodec.toOC((i + .5f) * rate / (2.f * size)) * 2.f;
    ...
    {
    	static int ls = 0;
    	Janalysis._analysis_output_always("noiseoff0", ls, noiseoffset[0], 0, n, true, false, 0);
    	Janalysis._analysis_output_always("noiseoff1", ls, noiseoffset[1], 0, n, true, false, 0);
    	Janalysis._analysis_output_always("noiseoff2", ls++, noiseoffset[2], 0, n, true, false, 0);
    }
    #endif */
  }

  final void _vp_psy_clear() {
    // int i, j;
    // if( p != null ) {
  ...
  private static final void seed_curve(
      final float[] seed,
      final float[][] curves,
      final float amp,
      final int oc,
  ...
  private static final void bark_noise_hybridmp(
      final int n,
      final int[] b,
      final float[] f,
      int f_offset,
  ...
  final void _vp_noisemask(final float[] logmdct, int offset, final float[] logmask) {
    ...
    /*if( false ) {// #if 0
    		{
    			...
    			if( (seq & 1) != 0 ) {
    				Janalysis._analysis_output("median2R", seq / 2, work, 0, n, true, false, 0);
    				Janalysis._analysis_output("envelope2R", seq / 2, work2, 0, n, true, false, 0);
    			} else {
    				Janalysis._analysis_output("median2L", seq/2, work, 0, n, true, false, 0);
    				Janalysis._analysis_output("envelope2L", seq / 2, work2, 0, n, true, false, 0);
    			}
  ...
  final void _vp_offset_and_mix(
    ...
    for (int i = 0; i < size; i++) {
      ...
      /* AoTuV */
      /**
       * @ M1 ** The following codes improve a noise problem. A fundamental idea uses the value of
       * masking and carries out the relative compensation of the MDCT. However, this code is not
       * perfect and all noise problems cannot be solved. by Aoyumi @ 2004/04/18
       */
      if (offset_select == 1) {
  ...
  private final Icomparator sort_comparator_set(final float[] dim) {
    this.mDim = dim;
    return this;
  }
  ...
  // private final Icomparator apsort = this;

  private static final void flag_lossless(
      final int limit,
      final float prepoint,
      final float postpoint,
      final float[] mdct,
      int mdct_offset,
      final float[] floor,
      final boolean[] flag,
  ...
  /**
   * Overload/Side effect: On input, the *q vector holds either the quantized energy (for elements
   * with the flag set) or the absolute values of the *r vector (for elements with flag unset). On
   * output, q holds the quantized energy for all elements
   */
  private final float noise_normalize(
      int limit,
      final float[] r,
      final float[] q,
      final float[] f,
      ...
      int length,
      final int[] out,
      final int iout_offset) {
    final Jvorbis_info_psy vinfo = this.vi;
    ...
    if (count != 0) {
      ...
      for (j = 0; j < count; j++) {
        ...
        if (acc >= vinfo.normal_thresh) {
          out[length + k] = r[k] < 0f ? -1 : 1; // java: extracted (int)Jcodec.unitnorm( r[k] );
          acc -= 1.f;
          q[k] = f[k];
        } else {
          out[length + k] = 0;
  ...
  /**
   * Noise normalization, quantization and coupling are not wholly seperable processes in depth>1
   * coupling.
   */
  final void _vp_couple_quantize_normalize(
      final int blobno,
      final Jvorbis_info_psy_global g,
      final Jvorbis_info_mapping0 vinfo,
      final float[][] mdct,
      final int[][] iwork,
    ...
    for (int i = 0; i < size; i += partition) {
      ...
      for (int k = 0; k < ch; k++) {
        ...
        if (nz[k]) {
          ...
          acc[track] =
              noise_normalize(limit, raw, quant, floor, null, offset_k, acc[track], i, jn, iout, i);
      ...
      for (int step = 0; step < coupling_steps; step++) {
        ...
        if (nz[Mi] || nz[Ai]) {
          ...
          acc[track] =
              noise_normalize(limit, raw, quant, floor, flag, oM, acc[track], i, jn, iM, i);
