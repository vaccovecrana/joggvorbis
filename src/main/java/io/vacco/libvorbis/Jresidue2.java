 ...
 * </pre>
 */
final class Jresidue2 extends Jresidue1 {
  ...
  // res0.c
  /**
   * designed for stereo or other modes where the partition size is an integer multiple of the
   * number of channels encoded in the current submap
   */
  private static int[][] _2class(
    ...
    for (int i = 0, l = info.begin / ch; i < partvals; i++) {
      ...
      for (; j < possible_partitions1; j++) {
        if (magmax <= classmetric1[j] && angmax <= classmetric2[j]) {
          break;
        }
      }
  ...
  /**
   * res2 is slightly more different; all the channels are interleaved into a single vector and
   * encoded.
   */
  final int forward(
      ...
      for (int s = 0, stages = look.stages; s < stages; s++) {
        ...
        for (int l = 0; i < partvals; l++) {
          ...
          for (int k = 0;
              ...
              k++, i += samples_per_partition) {
            final int p_k = p[k]; // java
            if ((secondstages[p_k] & s1) != 0) {
              final Jcodebook stagebook = partbooks[p_k][s];

              if (stagebook != null) {
                if (stagebook.vorbis_book_decodevv_add(in, i, ch, vb.opb, samples_per_partition)
                    == -1) {
                  return 0; // goto eopbreak;
                }
              }
