 ...
 * encode pointer will point to a decision tree. This is true of both VQ and huffman, but is mostly
 * useful with VQ.
 */
public final class Jstatic_codebook {
  ...
  // sharedbook.c
  /**
   * there might be a straightforward one-line way to do the below that's portable and totally safe
   * against roundoff, but I haven't thought of it. Therefore, we opt on the side of caution
   */
   ...
   *
   * <p>we need to deal with two map types: in map type 1, the values are generated algorithmically
   * (each column of the vector counts through the values in the quant vector). in map type 2, all
   * the values came in in an explicit list. Both value lists must be unpacked
   */
        ...
        case 1:
          ...
          for (int j = 0, je = this.entries; j < je; j++) {
            if ((sparsemap != null && length_list[j] != 0) || sparsemap == null) {
              ...
              int indexdiv = 1;
              // java: changed
              int off = sparsemap != null ? sparsemap[count] : count;
              off *= dimension;
  ...
  final void vorbis_staticbook_destroy() {
    if (allocedp != 0) {
      quantlist = null;
      lengthlist = null;
      clear();
  ...
  final int vorbis_staticbook_pack(final Joggpack_buffer opb) {
    int i, j;
    boolean ordered = false;

    /* first the basic parameters */
    opb.oggpack_write(0x564342, 24);
    opb.oggpack_write(this.dim, 16);
    final int entries_count = this.entries; // java
    opb.oggpack_write(entries_count, 24);
    ...
    if (ordered) {
      ...
      int count = 0;
      opb.oggpack_write(1, 1); /* ordered */
      opb.oggpack_write((int) length_list[0] - 1, 5); /* 1 to 32 */
  ...
  /**
   * unpacks a codebook from the packet buffer into the codebook struct, readies the codebook
   * auxiliary structures for decode
   */
      ...
      case 1:
        /* ordered */
        {
          ...
          for (int i = 0; i < entries; ) {
            ...
            if (length > 32 || num > entries - i || (num > 0 && (num - 1) >> (length - 1) > 1)) {
              s.vorbis_staticbook_destroy();
