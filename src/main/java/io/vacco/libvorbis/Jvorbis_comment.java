...
public final class Jvorbis_comment {
  ...
  public final void vorbis_comment_add(final String comment) {
    ...
    // FIXME why need extra null? see pack and unpack
  ...
  final int _vorbis_unpack_comment(final Joggpack_buffer opb) {
    ...
    if ((i < 0) || (i > ((opb.storage - opb.oggpack_bytes()) >> 2))) { // goto err_out;
      vorbis_comment_clear();
      return (Jcodec.OV_EBADHEADER);
    }
    ...
    for (i = 0; i < length; i++) {
      ...
      if ((len < 0) || (len > opb.storage - opb.oggpack_bytes())) { // goto err_out;
        vorbis_comment_clear();
        return (Jcodec.OV_EBADHEADER);
      }
      if (len != 0) {
