...
final class Jmdct_lookup {
  ...
  ********************************************************************/
  ...
  final void mdct_init(final int order) {
  ...
  private static void mdct_butterfly_8(final float[] x, final int offset) {
  ...
  private static void mdct_butterfly_16(final float[] x, final int offset) {
    int off0 = offset;
    ...
    mdct_butterfly_8(x, offset);
    mdct_butterfly_8(x, ++off0);
  }

  /** 32 point butterfly (in place, 4 register) */
  private static void mdct_butterfly_32(final float[] x, int offset) {
    int off16 = offset + 31;
    offset += 15;
    ...
    mdct_butterfly_16(x, offset);
    mdct_butterfly_16(x, off16);
  }

  /** N point first stage butterfly (in place, 2 register) */
  private static void mdct_butterfly_first(
      final float[] T, final float[] x, int offset, final int points) {
  ...
  private static void mdct_butterfly_generic(
      final float[] T, final float[] x, int offset, final int points, int trigint) {
    int x1 = offset + points; //        - 8;
    int x2 = offset + (points >> 1); // - 8;
    offset += 8; // java
  ...
  private final void mdct_butterflies(final float[] x, final int offset, int points) {
    final float[] T = this.trig;
    int stages = this.log2n - 5;

    if (--stages > 0) {
      mdct_butterfly_first(T, x, offset, points);
    }
    ...
    for (int i = 1; --stages > 0; i++) {
      ...
      for (int j = offset, je = offset + pi * (1 << i); j < je; j += pi) {
        mdct_butterfly_generic(T, x, j, pi, i4);
      }
    }
  ...
  final void mdct_clear() {
    // if( l != null ) {
    clear();
    // }
  }

  private final void mdct_bitreverse(final float[] x) {
  ...
  final void mdct_backward(final float[] in, final float[] out) {
    final int order = this.n;
    final int n2 = order >> 1;
  ...
  final void mdct_forward(final float[] in, final float[] out) {
    final int order = this.n;
    final int n2 = order >> 1;
    final int n4 = order >> 2;
