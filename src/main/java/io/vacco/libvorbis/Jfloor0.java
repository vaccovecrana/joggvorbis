 ...
 * </pre>
 */
final class Jfloor0 extends Jvorbis_func_floor {
  private static final class Jvorbis_look_floor0 extends Jvorbis_look_floor {
    private int ln = 0;
    private int m = 0;
    private int[][] linearmap = null;
    private final int[] n = new int[2];

    private Jvorbis_info_floor0 vi = null;
  ...
  */
  ...
  /*
  private static float vorbis_coslook(float a) {
  ...
  */
  ...
  /*
  private static float vorbis_invsqlook(float a) {
  ...
  */
  /** interpolated 1./sqrt(p) where .5 <= p < 1. */
  /*
  private static float vorbis_invsq2explook(int a) {
  ...
  */
  ...
  /*
  	private static float vorbis_fromdBlook(float a) {
  ...
  */
  /**
   * interpolated 1./sqrt(p) where .5 <= a < 1. (.100000... to .111111...) in 16.16 format
   *
   * <p>returns in m.8 format
   */
  /*
  private static int vorbis_invsqlook_i(int a, int e) {
  ...
  */
  /** interpolated lookup based fromdB function, domain -140dB to 0dB only a is in n.12 format */
  /*
  private static float vorbis_fromdBlook_i(int a) {
  ...
  */
  /**
   * interpolated lookup based cos function, domain 0 to PI only a is in 0.16 format, where 0==0,
   * 2^^16-1==PI, return 0.14
   */
  /*
  	private static int vorbis_coslook_i(int a) {
  ...
  */
  ...
  private static void vorbis_lsp_to_curve(
      final float[] curve,
      final int[] map,
      final int n,
      final int ln,
    ...
    /*if( FLOAT_LOOKUP ) {
    		...
    		while( i < n ) {
    			...
    			float p = .7071067812f;
    			float q = .7071067812f;
    			final float w = vorbis_coslook( wdel * k );
    			//float[] ftmp = lsp;
    			int ftmp = 0;
    			int c = m >> 1;
    ...
    /*if( INT_LOOKUP ) {
    		...
    		for( i = 0; i < m; i++ )
    			ilsp[i] = vorbis_coslook_i( (int)(lsp[i] / (float)Math.PI * 65536.f + .5f) );
    		...
    		while( i < n ) {
    			...
    			int qexp = 0, shift;
    			final int wi = vorbis_coslook_i( (k << 16) / ln );
    			...
    			amp = vorbis_fromdBlook_i( ampi *          n.4
    					vorbis_invsqlook_i( qi, qexp ) -   m.8, m+n<=8
    					ampoffseti );                              8.12[0]
    ...
    } else*/
    {
      ...
      while (i < n) {
        ...
        // q = Jcodec.fromdB( amp / (float)Math.sqrt(p + q) - ampoffset );
  ...
  */
  /**
   * Newton-Raphson-Maehly actually functioned as a decent root finder, but there are root sets for
   * which it gets into limit cycles (exacerbated by zero suppression) and fails. We can't afford to
   * fail, even if the failure is 1 in 100,000,000, so we now use Laguerre and later polish with
   * Newton-Raphson (which can then afford to fail)
   */
  /*
  ...
  */
  ...
  /*
  private static int vorbis_lpc_to_lsp(float[] lpc, float[] lsp, int m) {
  ...
  /*private static void floor0_free_info(vorbis_info_floor *i) {
  	Jvorbis_info_floor0 info = (Jvorbis_info_floor0)i;
  	if( info != null ) {
  		info.clear();
  		info = null;
  	}
  ...
  @Override
  final void pack(final Jvorbis_info_floor i, final Joggpack_buffer opg) {}
  ...
  final Jvorbis_info_floor unpack(final Jvorbis_info vi, final Joggpack_buffer opb) {
    ...
    final Jvorbis_info_floor0 info = new Jvorbis_info_floor0();
    info.order = opb.oggpack_read(8);
    info.rate = opb.oggpack_read(16);
    info.barkmap = opb.oggpack_read(16);
    info.ampbits = opb.oggpack_read(6);
    info.ampdB = opb.oggpack_read(8);
    info.numbooks = opb.oggpack_read(4) + 1;
    ...
    final Jstatic_codebook[] bp = ci.book_param; // java
    for (int j = 0, je = info.numbooks; j < je; j++) {
      final int books = opb.oggpack_read(8);
      info.books[j] = books;
  ...
  final Jvorbis_look_floor look(final Jvorbis_dsp_state vd, final Jvorbis_info_floor i) {
    final Jvorbis_info_floor0 info = (Jvorbis_info_floor0) i;
    final Jvorbis_look_floor0 look = new Jvorbis_look_floor0();
  ...
  final Object inverse1(final Jvorbis_block vb, final Jvorbis_look_floor i) {
    final Jvorbis_look_floor0 look = (Jvorbis_look_floor0) i;
    final Jvorbis_info_floor0 info = look.vi;

    final int ampraw = vb.opb.oggpack_read(info.ampbits);
    if (ampraw > 0) {
      /* also handles the -1 out of data case */
      final int maxval = (1 << info.ampbits) - 1;
      final float amp = (float) ampraw / maxval * info.ampdB;
      final int booknum = vb.opb.oggpack_read(Jcodec.ov_ilog(info.numbooks));

      if (booknum != -1 && booknum < info.numbooks) {
        /* be paranoid */
        final Jcodec_setup_info ci = vb.vd.vi.codec_setup;
        final Jcodebook b = ci.fullbooks[info.books[booknum]];
        float last = 0.f;
        ...
        if (b.vorbis_book_decodev_set(lsp, vb.opb, m) == -1) {
  ...
  /**
   * initialize Bark scale and normalization lookups. We could do this with static tables, but
   * Vorbis allows a number of possible combinations, so it's best to do it computationally.
   *
   * <p>The below is authoritative in terms of defining scale mapping. Note that the scale depends
   * on the sampling rate as well as the linear block and mapping sizes
   */
  private static void floor0_map_lazy_init(
      final Jvorbis_block vb, final Object infoX, final Jvorbis_look_floor0 look) {
    if (look.linearmap[vb.W] == null) {
      // final Jvorbis_dsp_state   vd = vb.vd;
      // final Jvorbis_info        vi = vd.vi;
      // final Jcodec_setup_info   ci = vi.codec_setup;
      final Jvorbis_info_floor0 info = (Jvorbis_info_floor0) infoX;
      final int W = vb.W;
      ...
      final float scale = look.ln / Jcodec.toBARK(info.rate / 2.f);
      ...
      for (j = 0; j < n; j++) {
        int val =
            (int)
                Math.floor(
                    Jcodec.toBARK(v * (float) j) * scale); /* bark numbers represent band edges */
        if (val >= look.ln) {
  ...
  final boolean inverse2(
      final Jvorbis_block vb, final Jvorbis_look_floor i, final Object memo, final float[] out) {
    final Jvorbis_look_floor0 look = (Jvorbis_look_floor0) i;
    final Jvorbis_info_floor0 info = look.vi;
    ...
    if (memo != null) {
      ...
      vorbis_lsp_to_curve(
          out, look.linearmap[vb.W], look.n[vb.W], look.ln, lsp, look.m, amp, (float) info.ampdB);
      return true;
    }
