...
public final class Jvorbis_info_psy_global {
  int eighth_octave_lines = 0;

  /* for block long/short tuning; encode only */
  final float[] preecho_thresh = new float[Jenvelope_lookup.VE_BANDS];
  final float[] postecho_thresh = new float[Jenvelope_lookup.VE_BANDS];
  ...
  final int[] coupling_pkHz = new int[Jvorbis_info.PACKETBLOBS];
  final int[][] coupling_pointlimit = new int[2][Jvorbis_info.PACKETBLOBS];
  final int[] coupling_prepointamp = new int[Jvorbis_info.PACKETBLOBS];
  final int[] coupling_postpointamp = new int[Jvorbis_info.PACKETBLOBS];
  final int[][] sliding_lowpass = new int[2][Jvorbis_info.PACKETBLOBS];
  //
  Jvorbis_info_psy_global() {}

  public Jvorbis_info_psy_global(
      final int eol,
      final float[] pre_thresh,
      final float[] post_thresh,
      final float penalty,
      final float preecho,
      final float ampmax,
      final int[] pkHz,
      final int[][] pointlimit,
      final int[] prepointamp,
      final int[] postpointamp,
      final int[][] lowpass) {
  ...
  final void copyFrom(final Jvorbis_info_psy_global g) {
    ...
    for (int i = 0; i < coupling_pointlimit.length; i++) {
      System.arraycopy(
          g.coupling_pointlimit[i], 0, coupling_pointlimit[i], 0, coupling_pointlimit.length);
    }
    System.arraycopy(
        g.coupling_prepointamp, 0, coupling_prepointamp, 0, coupling_prepointamp.length);
    System.arraycopy(
        g.coupling_postpointamp, 0, coupling_postpointamp, 0, coupling_postpointamp.length);
