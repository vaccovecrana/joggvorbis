 ...
 * </pre>
 */
class Jresidue0 extends Jvorbis_func_residue implements Idecodepart {
  ...
  Jresidue0(final boolean isPack, final boolean isClass, final boolean isForward) {
  ...
  /**
   * returns 0 on OK or -1 on eof decode vector / dim granularity gaurding is done in the upper
   * layer
   */
  @Override
  public int decodepart(
      final Jcodebook book, final float[] a, final int offset, final Joggpack_buffer b, int n) {
  ...
  final void free_look(final Jvorbis_look_residue i) {
    if (i != null) {

      /* if( TRAIN_RES ) {
      			{
      				int j, k, l;
      				final Jvorbis_look_residue0 look = (Jvorbis_look_residue0)i;
      				for( j = 0; j < look.parts; j++ ) {
      					...
      					for( k = 0; k < 8; k++ )
      						if( look.training_data[k][j] != 0 ) {
      							...
      							try {
      								Jcodebook statebook = look.partbooks[j][k];
      ...
      // Jvorbis_info_residue0 info = look.info;
  ...
  @Override
  // res0_pack(vorbis_info_residue *vr, Joggpack_buffer opb)
  final void pack(final Jvorbis_info_residue vr, final Joggpack_buffer opb) {
    final Jvorbis_info_residue0 info = (Jvorbis_info_residue0) vr;
    int j, acc = 0;
    opb.oggpack_write(info.begin, 24);
    opb.oggpack_write(info.end, 24);

    opb.oggpack_write(info.grouping - 1, 24); /* residue vectors to group and
														code with a partitioned book */
    final int partitions = info.partitions; // java
    opb.oggpack_write(partitions - 1, 6); /* possible partition choices */
    opb.oggpack_write(info.groupbook, 8); /* group huffman book */
    ...
    for (j = 0; j < partitions; j++) {
      final int s = tmp[j];
      if (Jcodec.ov_ilog(s) > 3) {
        /* yes, this is a minor hack due to not thinking ahead */
        opb.oggpack_write(s, 3);
        opb.oggpack_write(1, 1);
        opb.oggpack_write(s >> 3, 5);
      } else {
        opb.oggpack_write(s, 4); /* trailing zero */
      }
      acc += icount(s);
    }
    tmp = info.booklist; // java
    for (j = 0; j < acc; j++) {
      opb.oggpack_write(tmp[j], 8);
    }
  }
  ...
  final Jvorbis_info_residue unpack(final Jvorbis_info vi, final Joggpack_buffer opb) {
    final Jvorbis_info_residue0 info = new Jvorbis_info_residue0();
    final Jcodec_setup_info ci = vi.codec_setup;

    info.begin = opb.oggpack_read(24);
    info.end = opb.oggpack_read(24);
    info.grouping = opb.oggpack_read(24) + 1;
    info.partitions = opb.oggpack_read(6) + 1;
    info.groupbook = opb.oggpack_read(8);
    ...
    int j, acc = 0;
    for (j = 0; j < partitions; j++) {
      int cascade = opb.oggpack_read(3);
      final int cflag = opb.oggpack_read(1);
      if (cflag < 0) {
        return null; // goto errout;
      }
      if (cflag != 0) {
        final int c = opb.oggpack_read(5);
        if (c < 0) {
          return null; // goto errout;
        }
        cascade |= (c << 3);
    ...
    tmp = info.booklist; // java
    for (j = 0; j < acc; j++) {
      final int book = opb.oggpack_read(8);
      if (book < 0) {
        return null; // goto errout;
      }
      tmp[j] = book;
    ...
    allow an exploit */
    {
      final Jstatic_codebook codebok = cb[info.groupbook]; // java
  ...
  final Jvorbis_look_residue look(final Jvorbis_dsp_state vd, final Jvorbis_info_residue vr) {
    final Jvorbis_info_residue0 info = (Jvorbis_info_residue0) vr;
    final Jvorbis_look_residue0 look = new Jvorbis_look_residue0();
    final Jcodec_setup_info ci = vd.vi.codec_setup;
    ...
    for (int j = 0; j < parts; j++) {
      ...
      final int stages = Jcodec.ov_ilog(s);
      if (stages != 0) {
        if (stages > maxstage) {
          maxstage = stages;
        }
        partbooks[j] = new Jcodebook[stages];
