package io.vacco.joggvorbis.spi.convert;
...
public final class OggVorbisFormatConversionProvider extends FormatConversionProvider {
  ...
  public AudioFormat[] getTargetFormats(
      final Encoding targetEncoding, final AudioFormat sourceFormat) {
    ...
    if (sourceFormat.getEncoding().equals(ENCODING)) {
      ...
      if (targetEncoding == Encoding.PCM_UNSIGNED) {
        final AudioFormat af[] = {
          new AudioFormat(
              sourceFormat.getSampleRate(), 16, sourceFormat.getChannels(), false, false),
          new AudioFormat(
              sourceFormat.getSampleRate(), 16, sourceFormat.getChannels(), false, true),
  ...
  public AudioInputStream getAudioInputStream(
      final Encoding targetEncoding, final AudioInputStream sourceStream) {

    final AudioFormat saf = sourceStream.getFormat();
    final AudioFormat taf =
        new AudioFormat(
            targetEncoding,
            saf.getSampleRate(),
            16,
  ...
  public AudioInputStream getAudioInputStream(
    ...
    return new OggVorbisDecodedAudioInputStream(
        sourceStream, targetFormat, AudioSystem.NOT_SPECIFIED);
  }
}
