...
public final class OggVorbisDecodedAudioInputStream extends AudioInputStream {
  ...
  //
  public OggVorbisDecodedAudioInputStream(
      final InputStream stream, final AudioFormat format, final long length) {
    super(stream, format, length);
    mIsBigEndian = format.isBigEndian();
    mIsSigned = format.getEncoding() == Encoding.PCM_SIGNED;
    if (mOVF.ov_open_callbacks(stream, null, 0, JOggVorbis_File.OV_CALLBACKS_STREAMONLY_NOCLOSE)
        < 0) {
      mOVF.ov_clear();
    }
  }
