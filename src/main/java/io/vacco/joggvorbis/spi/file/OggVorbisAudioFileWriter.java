...
public final class OggVorbisAudioFileWriter extends AudioFileWriter {
  ...
  public int write(final AudioInputStream stream, final Type fileType, final OutputStream out)
    ...
    if (is_vbr) {
      if (vi.vorbis_encode_init_vbr(channels, (int) stream.getFormat().getSampleRate(), quality)
          < 0) {
        throw new IOException("Can't init encoder");
      }
    } else {
      if (vi.vorbis_encode_init(
              channels, (int) stream.getFormat().getSampleRate(), -1, (int) (quality * 1000f), -1)
          < 0) {
        throw new IOException("Can't init encoder");
      }
    }
          ...
          if (is_big_endian) {
            for (int c = 0; c < channels; c++) {
              ...
              for (i = c << 1; i < bytes; i += channels2) {
                p[dest++] =
                    ((float) (((int) readbuffer[i] << 8) | (0x00ff & (int) readbuffer[i + 1])))
                        / 32768.f;
              }
            }
          } else {
            for (int c = 0; c < channels; c++) {
              ...
              for (i = c << 1; i < bytes; i += channels2) {
                p[dest++] =
                    ((float) (((int) readbuffer[i + 1] << 8) | (0x00ff & (int) readbuffer[i])))
                        / 32768.f;
              }
            }
          }
  ...
  @Override
  public int write(final AudioInputStream stream, final Type fileType, final File file)
      throws IOException {
    if (!fileType.equals(OGGVORBIS[0])) {
      throw new IllegalArgumentException();
    }
