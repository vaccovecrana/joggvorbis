package io.vacco.joggvorbis.spi.file;

import io.vacco.joggvorbis.spi.convert.OggVorbisFormatConversionProvider;
...
public final class OggVorbisAudioFileReader extends AudioFileReader {
  // there is a real problem: a decoder must process all metadata block. this block can have a huge
  // size.
  ...
  public AudioFileFormat getAudioFileFormat(final InputStream stream)
    ...
    if (ovf.ov_open_callbacks(stream, null, 0, JOggVorbis_File.OV_CALLBACKS_STREAMONLY_NOCLOSE)
        < 0) {
      ovf.ov_clear();
    ...
    final AudioFormat af =
        new AudioFormat(
            OggVorbisFormatConversionProvider.ENCODING,
            vi.rate,
  ...
  public AudioInputStream getAudioInputStream(final InputStream stream)
      throws UnsupportedAudioFileException, IOException {
    // doc says: If the input stream does not support this, this method may fail with an
    // IOException.
    // if( ! stream.markSupported() ) stream = new BufferedInputStream( stream,
    // JOggVorbis_File.CHUNKSIZE * 2 );// possible resources leak
    try {
