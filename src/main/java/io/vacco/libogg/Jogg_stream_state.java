...
public final class Jogg_stream_state {
  ...
  int[] lacing_vals;
  /**
   * granulepos values for headers. Not compact this way, but it is simple coupled to the lacing
   * fifo
   */
  long[] granule_vals;
  ...
  private final int ogg_stream_iovecin(
      final Jogg_iovec_t[] iov, final int count, final boolean eos, final long granulePos) {
  ...
  /**
   * Conditionally flush a page; force==0 will only flush nominal-size pages, force==1 forces us to
   * flush a page regardless of page size so long as there's any data available at all.
   */
  ...
  /**
   * This will flush remaining packets into a page (returning nonzero), even if there is not enough
   * data to trigger a flush normally (undersized page). If there are no packets or partial packets
   * to flush, ogg_stream_flush returns 0. Note that ogg_stream_flush will try to flush a normal
   * sized page like ogg_stream_pageout; a call to ogg_stream_flush does not guarantee that all
   * packets have flushed. Only a return value of 0 from ogg_stream_flush indicates all packet data
   * is flushed into pages.
   *
   * <p>since ogg_stream_flush will flush the last page in a stream even if it's undersized, you
   * almost certainly want to use ogg_stream_pageout (and *not* ogg_stream_flush) unless you
   * specifically need to flush a page regardless of size in the middle of a stream.
   */
  ...
  /**
   * Like the above, but an argument is provided to adjust the nominal page size for applications
   * which are smart enough to provide their own delay based flushing
   */
  ...
  /**
   * This constructs pages from buffered packet segments. The pointers returned are to static
   * buffers; do not free. The returned buffers are good only until the next call (using the same
   * ogg_stream_state)
   */
  public final int ogg_stream_pageout(final Jogg_page og) {
    ...
    if ((this.e_o_s && this.lacing_fill != 0)
        || /* 'were done, now flush' case */ (this.lacing_fill != 0 && !this.b_o_s)) {
      force = true;
    }
  ...
  /**
   * add the incoming page to the stream state; we decompose the page into packet segments here as
   * well.
   */
