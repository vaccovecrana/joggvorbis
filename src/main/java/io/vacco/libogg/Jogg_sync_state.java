...
public final class Jogg_sync_state {
  ...
  /**
   * This has two layers to place more of the multi-serialno and paging control in the application's
   * hands. First, we expose a data buffer using ogg_sync_buffer(). The app either copies into the
   * buffer, or passes it directly to read(), etc. We then call ogg_sync_wrote() to tell how many
   * bytes we just added.
   *
   * <p>Pages are returned (pointers into the buffer in ogg_sync_state) by ogg_sync_pageout(). The
   * page is then submitted to ogg_stream_pagein() along with the appropriate ogg_stream_state* (ie,
   * matching serialno). We then get raw packets out calling ogg_stream_packetout() with a
   * ogg_stream_state.
   */
   ...
   *
   * <p>return values for this: -n) skipped n bytes 0) page not ready; more data (no bytes skipped)
  ...
  public final int ogg_sync_pageseek(final Jogg_page og) {
    ...
    if (this.headerbytes == 0) {
      ...
      if (d[page] != 'O' || d[page + 1] != 'g' || d[page + 2] != 'g' || d[page + 3] != 'S') {
  ...
  /**
   * sync the stream and get a page. Keep trying until we find a page. Suppress 'sync errors' after
   * reporting the first.
   *
   * <p>return values: -1) recapture (hole in data) 0) need more data 1) page returned
   *
   * <p>Returns pointers into buffered data; invalidated by next call to _stream, _clear, _init, or
   * _buffer
   */
